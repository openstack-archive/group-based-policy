...
class GroupPolicyPlugin(group_policy_mapping_db.GroupPolicyMappingDbPlugin):
    ...
    usage_graph = {'l3_policy': {},
                   ...
                   'endpoint': {'endpoint_group_id': 'endpoint_group'}}
    _plurals = None
    ...
    def _validate_shared_update(self, context, original, updated, identity):
        ...
        if updated.get('shared'):
            # False to True, it's just like creating a new shared resource
    ...
    def _check_shared_or_different_tenant(self, context, obj,
        ...
        for ref in refs:
            if ref.get('shared') or tenant_id != ref['tenant_id']:
                raise gp_exc.InvalidSharedAttributeUpdate(
                    id=obj['id'], rid=[ref['id']])
    ...
    def _validate_l2_policy_unshare(self, context, obj):
        self._check_shared_or_different_tenant(
            context, obj, self.get_endpoint_groups, 'l2_policy_id')

    def _validate_endpoint_group_unshare(self, context, obj):
        self._check_shared_or_different_tenant(
            context, obj, self.get_endpoints, 'endpoint_group_id')

    def _validate_network_service_policy_unshare(self, context, obj):
        self._check_shared_or_different_tenant(
            context, obj, self.get_endpoint_groups,
            'network_service_policy_id')

    def _validate_contract_unshare(self, context, obj):
        self._check_shared_or_different_tenant(
            context, obj, self.get_endpoint_groups, 'id',
            obj['providing_endpoint_groups'] +
            obj['consuming_endpoint_groups'])
    ...
    def _validate_policy_rule_unshare(self, context, obj):
        c_ids = self._get_policy_rule_contracts(context, obj['id'])
        self._check_shared_or_different_tenant(
            context, obj, self.get_contracts, 'id', c_ids)
    ...
    def create_endpoint_group(self, context, endpoint_group):
        ...
        with session.begin(subtransactions=True):
            ...
            self._validate_shared_create(context, result, 'endpoint_group')
        ...
        with session.begin(subtransactions=True):
            ...
            updated_endpoint_group = super(GroupPolicyPlugin,
                                           self).update_endpoint_group(
                                               context, endpoint_group_id,
                                               endpoint_group)
            self._validate_shared_update(context, original_endpoint_group,
                                         updated_endpoint_group,
                                         'endpoint_group')
    ...
    def create_contract(self, context, contract):
        ...
        with session.begin(subtransactions=True):
            result = super(GroupPolicyPlugin, self).create_contract(context,
                                                                    contract)
            self._validate_shared_create(context, result, 'contract')
        ...
        with session.begin(subtransactions=True):
            ...
            updated_contract = super(GroupPolicyPlugin,
                                     self).update_contract(context, id,
                                                           contract)
            self._validate_shared_update(context, original_contract,
