...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def create_external_segment_precommit(self, context):
        if context.current['subnet_id']:
            ...
            db_es.encap_type = network.get('provider:network_type')
            db_es.encap_value = network.get('provider:segmentation_id')

    def create_external_segment_postcommit(self, context):
        # Use implicit external subnet if needed.
        # Everything was taken care of in the precommit for the explicit case
        if not context.current['subnet_id']:
            self._use_implicit_external_subnet(context)

    def update_external_segment_precommit(self, context):
        invalid = ['cidr', 'ip_version', 'encap_type', 'encap_value',
                   'subnet_id', 'port_address_translation']
        for attr in invalid:
    ...
    def delete_external_segment_postcommit(self, context):
        if context.current['subnet_id']:
            self._cleanup_subnet(context._plugin_context,
                                 context.current['subnet_id'], None)
    ...
    def _use_implicit_external_subnet(self, context):
        kwargs = {'name': 'es_' + context.current['name'],
                  'router:external': True,
                  'provider:network_type': context.current['encap_type'],
                  'provider:segmentation_id': context.current['encap_value']}
        network = self._create_implicit_network(context, **kwargs)
        attrs = {'tenant_id': context.current['tenant_id'],
                 'name': 'es_' + context.current['name'],
        ...
        self._mark_subnet_owned(
            context._plugin_context.session, subnet['id'])
        ...
        if context.current['routers']:
            ...
            for es in es_list:
                ...
                interface_info = {
                    ...
                    'external_fixed_ips': es_dict[es['id']] or
                    attributes.ATTR_NOT_SPECIFIED}
                self._add_router_gw_interface(context._plugin_context,
    ...
    def _handle_policy_rule_sets(self, context):
        ...
        self._set_sg_rules_for_subnets(context, subnets, provided_policy_rule_sets,
                              consumed_policy_rule_sets)
    ...
    def _manage_policy_rule_set_rules(self, context, policy_rule_set,
        ...
        policy_rule_set_sg_mappings = self._get_policy_rule_set_sg_mapping(
            context._plugin_context.session, policy_rule_set['id'])
        policy_rule_set_db = context._plugin._get_policy_rule_set(
            context._plugin_context, policy_rule_set['id'])
        ptg_mapping = self._get_policy_rule_set_ptg_mapping(
            context, policy_rule_set_db)
        cidr_mapping = self._get_ptg_cidrs_mapping(context, ptg_mapping)
