...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def _handle_network_service_policy(self, context):
        ...
        network_service_policy_id = context.current.get("network_service_policy_id")
    ...
    def delete_network_service_policy_postcommit(self, context):
        #TODO: Get all EPGs using the policy and do a cleanup
        import pdb;pdb.set_trace()
                ...
                if not epgs_consuming_contract or not epgs_providing_contract:
                    ...
                    for rule_id in contract.get('policy_rules'):
                        rule_chain_map = self._get_rule_servicechain_mapping(
                                    context._plugin_context.session,
                                    rule_id)
                        if not rule_chain_map or not (
                                rule_chain_map.servicechain_instance_id):
                            break  # Only one redirect action per rule
        ...
        for allocation_pool in allocation_pools:
            ...
            if ip_address == str(netaddr.IPAddress(pool_end_ip) + 1):
                ...
                subnet = self._update_subnet(context._plugin_context,
                                             subnet['id'], subnet)
                return
        #TODO(Magesh):Have to test this logic. Add unit tests
        subnet['allocation_pools'].apend({"start": ip_address,
                                          "end": ip_address})
        del subnet['gateway_ip']
        subnet = self._update_subnet(context._plugin_context,
                                     subnet['id'], subnet)
        #TODO(Magesh): Better to add logic to Merge the pools together after
        #Fragmentation
    ...
    def _create_servicechain_instance(self, context, servicechain_spec,
        ...
        if ip_single:
            key = ip_single['name']
            servicepolicy_ip = self._get_service_policy_ipaddress(
                                        self, context, provider_epg)
