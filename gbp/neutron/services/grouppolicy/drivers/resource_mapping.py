...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def _disassoc_sgs_from_ep(self, context, ep, sg_list, lookup_ep=True):
        if lookup_ep:
            ep = context._plugin.get_endpoint(context._plugin_context, ep)
        port_id = ep['port_id']
        port = self._core_plugin.get_port(context._plugin_context, port_id)
        cur_sg_list = port[ext_sg.SECURITYGROUPS]
    ...
    def _disassoc_epg_sg_from_ep(self, context, ep, epg_id):
        sg_list = self._generate_list_of_sg_from_epg(context, epg_id)
        self._disassoc_sgs_from_ep(context, ep, sg_list, lookup_ep=False)
