...
class ApicMappingDriver(api.ResourceMappingDriver):
    ...
    def gbp_plugin(self):
        if not self._gbp_plugin:
            self._apic_gbp = manager.NeutronManager.get_service_plugins().get(
                "GROUP_POLICY")
    ...
    def process_path_deletion(self, context, port):
        port_details = self.apic_gbp.get_gbp_details(
            context, port_id=port['id'], host=port['host_id'])
    ...
    def _manage_endpoint_port(self, plugin_context, ep):
        ...
        if port.get('host_id'):
            port_details = self.apic_gbp.get_gbp_details(
                plugin_context, port_id=port['id'], host=port['host_id'])
            if port_details:
                ...
                with self.apic_manager.apic.transaction() as trs:
                    self.apic_manager.ensure_path_created_for_port(
                        tenant_id, epg, port['host_id'], seg, bd_name=bd,
                        transaction=trs)
    ...
    def _manage_epg_subnets(self, plugin_context, epg, added_subnets,
        ...
        mapped_tenant = self.name_mapper.tenant(plugin_context,
                                                epg['tenant_id'])
        mapped_l2p = self.name_mapper.endpoint_group(plugin_context,
                                                     epg['l2_policy_id'])
    ...
    def _port_id_to_ep(self, context, port_id):
        eps = self.gbp_plugin.get_endpoints(
