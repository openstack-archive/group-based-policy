...
class OdlMappingDriver(api.ResourceMappingDriver):
    ...
    def create_policy_target_postcommit(self, context):
        ...
        for fixed_ip in fixed_ips:
            l3-address.append(
                {
                 "ip_address": fixed_ip['ip-address'],
                 "l3-context": l3ctx_id
                }
            )
        eps = [{
            "endpoint-group": ptg_id,
            "l2-context": l2ctx_id,
            "l3-address": l3-address,
            "mac-address": mac_address,
            "neutron-port-id": neutron_port_id,
            "tenant": tenant_id
        }]
        self.odl_manager.register_eps(eps)
    ...
    def delete_policy_target_postcommit(self, context):
        ...
        mac_address = port['mac_address']
        ptg = self.gbp_plugin.get_policy_target_group(context,
                                                      context.current['policy_target_group_id'])
        l2ctx_id = ptg['l2_policy_id']
        #TODO: also need l3ctx info
