...
#TODO:Remove update support for now till its properly added
#Raise an exception in update_precommit
class ServiceChainInstancePolicyMap(model_base.BASEV2):
    """NVSD Policy attached to the Service Chain Instance."""

    __tablename__ = 'nvsd_sc_instance_policy'
...
class OneconvergenceServiceChainDriver(simplechain_driver.SimpleChainDriver, periodic_task.PeriodicTasks):
    ...
    def create_servicechain_instance_postcommit(self, context):
        ...
        pendinginsertion = PendingServiceChainInsertions(thread_context,
                                                         node_stacks,
                                                         context.current['id'],
                                                         context.current['provider_epg'],
                                                         context.current['consumer_epg'],
                                                         context.current['classifier'])
        #FIXME(Magesh): periodic_tasks doesnt work. So using threads instead
        #self.pending_chain_insertions.append(pendinginsertion)
    ...
    def _update_servicechain_instance(self, context, sc_instance, newspec):
        ...
        for node_stack in node_stacks:
            stack_resources = HeatClient(context._plugin_context).resources.list(node_stack.stack_id)
    ...
    def _delete_chain_policy_map(self, session, sc_instance_id):
        with session.begin(subtransactions=True):
            policy_id = session.query(ServiceChainInstancePolicyMap
    ...
    def _get_chain_policy_map(self, session, sc_instance_id):
        with session.begin(subtransactions=True):
            chain_policy_map = session.query(ServiceChainInstancePolicyMap
                                      ).filter_by(instance_id=sc_instance_id
                                                  ).one()
        return chain_policy_map

    #FIXME(Magesh): This doesnt work. So using threads instead
    @periodic_task.periodic_task(run_immediately=True)
    def periodic_chain_processing_task(self):
        try:
            if self.pending_chain_insertions:
                #TODO(magesh): Avoid copying the list
                for pending_chain in self.pending_chain_insertions[:]:
                    if(self._perform_service_insertion(pending_chain)):
                        self.pending_chain_insertions.remove(pending_chain)
        except Exception as err:
            LOG.error(_("Failed to process a pending Service Chain Insertion "
                        "Error : %s"), err)
    ...
    def create_nvsd_policy(self, context, left_group, right_group,
        ...
        rule_ids = []
        #FIXME:NVSD doesnt accept multiple actions, but still needs actions
        #as a list
    ...
    def delete_nvsd_policy(self, context, sc_instance_id):
        ...
        self.nvsd_api.delete_policy(context._plugin_context,
                                    nvsd_policy_id)
        #TODO(Magesh): Check if this sequence works on NVSD
        for rule_id in nvsd_policy.get("rules"):
            rule = self.nvsd_api.get_policy_rule(context._plugin_context, rule_id)
            self.nvsd_api.delete_policy_rule(context._plugin_context, rule_id)
            for action_id in rule.get("actions"):
                self.nvsd_api.delete_policy_action(context._plugin_context, action_id)    
    ...
    def _fetch_serviceids_from_stack(self, context, node_stacks,
                                     chain_instance_id):
        #TODO(Magesh): Storing Service IDs in DB required ??
        service_ids = []
        for node_stack in node_stacks:
            ...
            if service['insertion_mode'] == "L2":
                if not l2redirect_action:
                    l2redirect_action = {"action_type": "L2REDIRECT",
                                         ...
                                         'user_id': context.user,
                                         "action_value_list": [{"service": service_id}]}
                else:
                    l2redirect_action['action_value_list'].append({"service": service_id})
    ...
    def _perform_service_insertion(self, pending_chain):
        ...
        if status == self.CREATE_IN_PROGRESS:
            ...
            return True
        #TODO(Magesh): Service itself might be in PENDING_CREATE.
        #We should either wait here or in Heat !!
        ...
        policy_id = self.create_nvsd_policy(context, left_group, right_group,
                                            classifier_id, nvsd_action_list)
