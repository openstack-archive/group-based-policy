...
class GroupPolicyExtensionTestCase(test_api_v2_extension.ExtensionTestCase):
    ...
    def _get_create_network_service_policy_attrs(self):
        return {'name': 'nsp1', 'tenant_id': _uuid(),
                ...
                'network_service_params': {'ip_single':
                                           {'name': 'vip',
                                            'value': 'self_subnet'}}}
...
class TestGroupPolicyAttributeValidators(base.BaseTestCase):
    ...
    def test_validate_network_service_params(self):
        test_params = {'ip_single':
                       {'name': 'vip_internal', 'value': 'self_subnet'}}
        self.assertIsNone(gp._validate_network_svc_params(test_params))
        test_params = {'ip_pool':
                       {'name': 'vip_internal', 'value': 'external_subnet'}}
        self.assertIsNone(gp._validate_network_svc_params(test_params))
        test_params = {'string': {'name': 'abc', 'value': 'xyz'}}
        self.assertIsNone(gp._validate_network_svc_params(test_params))

    def test_validate_network_service_params_not_a_dict(self):
        test_params = 'ip'
        msg = gp._validate_network_svc_params(test_params)
        self.assertEqual(
            msg, "'ip' is not a dictionary")

    def test_validate_network_service_params_bad_type(self):
        test_params = {'ip_': {'name': 'vip', 'value': 'self_subnet'}}
        msg = gp._validate_network_svc_params(test_params)
        self.assertEqual(
            msg, "Network service param type(s) 'ip_' not supported")

    def test_validate_network_service_params_bad_key(self):
        test_params = {'ip_pool': {'n': 'vip', 'value': 'self_subnet'}}
        msg = gp._validate_network_svc_params(test_params)
        self.assertEqual(
            msg, "Unknown key(s) 'n' in network service params")

    def test_validate_network_service_params_bad_value(self):
        test_params = {'ip_pool': {'name': 'vip', 'value': 'subnet'}}
        msg = gp._validate_network_svc_params(test_params)
        self.assertEqual(
            msg, "Network service param value 'subnet' is not supported")

    def test_validate_network_service_params_no_dict_value(self):
        test_params = {'ip_pool': 'vip'}
        msg = gp._validate_network_svc_params(test_params)
        self.assertEqual(
            msg, "Network service params require a dictionary with name "
                 "and value, 'vip' is not a dictionary")
