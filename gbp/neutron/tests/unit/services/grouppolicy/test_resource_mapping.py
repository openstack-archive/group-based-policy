...
class TestPolicyRuleSet(ResourceMappingTestCase):
    ...
    def test_hierarchial_redirect(self):
        ...
        parent_classifier = self.create_policy_classifier(
            name="class2", protocol="tcp", direction="in", port_range="80")
        parent_classifier_id = parent_classifier['policy_classifier']['id']
        ...
        parent_policy_rule = self.create_policy_rule(
            name='pr1', policy_classifier_id=parent_classifier_id,
        ...
        self.assertEqual(sc_instance['port_range'], "80:80")
        self.assertEqual(sc_instance['direction'], "in")
        self.assertEqual(sc_instance['protocol'], "tcp")
        req = self.new_delete_request(
            'policy_target_groups', consumer_ptg_id)
        res = req.get_response(self.ext_api)
        self.assertEqual(res.status_int, webob.exc.HTTPNoContent.code)
