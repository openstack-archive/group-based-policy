...
def _get_admin_auth_url():
    """
    Get the admin keystone URL from the right directive.
    """
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron admin_auth_url".split(' '))
    except:
        pass
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron auth_url".split(' '))
    except:
        print("Error getting admin url")
        sys.exit(1)


def _get_admin_user_name()
    """
    Pick the admin user name from the right directive.
    """
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron admin_username".split(' '))
    except:
        pass
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron username".split(' '))
    except:
        pass
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron admin_user".split(' '))
    except:
        print("Error getting admin user name")
        sys.exit(1)

def _get_admin_password()
    """
    Pick the admin user password from the right directive.
    """
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron admin_password".split(' '))
    except:
        pass
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron password".split(' '))
    except:
        print("Error getting admin user password")
        sys.exit(1)

def _get_admin_tenant()
    """
    Pick the admin tenant from the right directive.
    """
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron admin_tenant_name".split(' '))
    except:
        pass
    try:
        return subprocess.check_output("crudini --get /etc/nova/nova.conf neutron project_name".split(' '))
    except:
        print("Error getting admin user password")
        sys.exit(1)
...
def create_nfp_namespace_file():
    ...
    auth_url = _get_admin_auth_url()
    filepx.write("\n\tAUTH_URI="+$auh_url)
    user_name = _get_admin_user_name()
    filepx.write("\n\tADMIN_USER="+$user_name)
    password = _get_admin_password()
    filepx.write("\n\tADMIN_PASSWD="+$password)
    tenant_name = _get_admin_tenant()
    filepx.write("\n\tADMIN_TENANT_NAME="+$tenant_name)

    try:
        domain_name = subprocess.check_output("crudini --get /etc/nova/nova.conf neutron project_domain_name".split(' '))
        filepx.write("\n\tADMIN_PROJECT_DOMAIN_NAME="+$domain_name)
        filepx.write("\n\texport OS_PROJECT_DOMAIN_NAME=$ADMIN_PROJECT_DOMAIN_NAME")
    except:
        pass

    try:
        domain_name =  subprocess.check_output("crudini --get /etc/nova/nova.conf neutron user_domain_name".split(' '))
        filepx.write("\n\tADMIN_USER_DOMAIN_NAME="+$domain_name)
