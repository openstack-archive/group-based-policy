...
from gbpservice.contrib.nfp.config_orchestrator.handlers import config
...
class FirewallTestCase(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.conf = Conf()
        self.fw_handler = config.firewall.FwAgent(self.conf, 'sc')
...
class LoadBalanceTestCase(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.conf = Conf()
        self.lb_handler = config.loadbalancer.LbAgent(self.conf, 'sc')
        self.context = TestContext().get_context()
