...
from gbpservice.contrib.nfp.configurator.drivers.vpn import vyos
from gbpservice.contrib.tests.unit.nfp.configurator import test_data
...
bdobj.register_agent_object_with_driver(
    'agent',
    vpn.VpnaasRpcSender(test_data.vpn_test_data.VPNTestData().sc))
...
class VpnaasIpsecDriverTestCase(base.BaseTestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conf = 'conf'
        self.test_dict = test_data.vpn_test_data.VPNTestData()
        self.context = self.test_dict.make_service_context()
        self.plugin_rpc = vpn.VpnaasRpcSender(self.test_dict.sc)
        self.driver = vyos.vyos_vpn_driver.VpnaasIpsecDriver(self.conf)
        self.svc_validate = (
            vyos.vyos_vpn_driver.VPNServiceValidator(self.plugin_rpc))
        self.resp = mock.Mock()
...
class VpnGenericConfigDriverTestCase(base.BaseTestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.context = self.test_dict.make_service_context()
        self.plugin_rpc = vpn.VpnaasRpcSender(self.test_dict.sc)
        self.rest_apt = vyos.vyos_vpn_driver.RestApi(self.test_dict.vm_mgmt_ip)
        self.driver = vyos.vyos_vpn_driver.VpnGenericConfigDriver()
        self.resp = mock.Mock()
        self.fake_resp_dict = {'status': True}
...
class VPNSvcValidatorTestCase(base.BaseTestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.test_dict = test_data.vpn_test_data.VPNTestData()
        self.plugin_rpc = vpn.VpnaasRpcSender(self.test_dict.sc)
        self.valid_obj = vyos.vyos_vpn_driver.VPNServiceValidator(
                                                        self.plugin_rpc)
...
class RestApiTestCase(base.BaseTestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rest_obj = vyos.vyos_vpn_driver.RestApi((
            test_data.vpn_test_data.VPNTestData().vm_mgmt_ip))
        self.resp = mock.Mock()
        self.resp = mock.Mock(status_code=200)
        self.test_dict = test_data.vpn_test_data.VPNTestData()
