...
def main():
    ...
    #if test.test_gbp_prs_pr_shared_func_3()==0:
    #   test.cleanup(tc_name='TESTCASE_GBP_PRS_PR_SHARED_INTEG_3')
    #if test.test_gbp_prs_pr_shared_func_4()==0:
    #   test.cleanup(tc_name='TESTCASE_GBP_PRS_PR_SHARED_INTEG_4')
    test.cleanup()
    report_results('test_gbp_prs_pr_shared_func','test_results.txt')
    sys.exit(1)
...
class test_gbp_prs_pr_shared_func(object):
    ...
    def test_gbp_prs_pr_shared_func_1(self):
        ...
        ruleset_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'ruleset','grppol_prs_many',\
                                         ...
                                         description="'For devstack demo'",shared='False')
        if ruleset_uuid == 0:
           self._log.info("\nStep 2: Creating Policy RuleSet with multiple PRs(shared=True) and shared=False , Failed" )
           return 0
        ## Verify starts
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset',ruleset_uuid,name='grppol_prs_many',\
                                                description='For devstack demo',shared='False')==0:
           self._log.info("# Step 2B: Verify Policy RuleSet and its shared='False' == Failed")
           return 0        
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2,'ruleset','grppol_prs_many',shared='True') != 0:
           self._log.info("# Step 3: Updating Policy RuleSet's Attribute shared=True DID NOT Fail" )
           return 0
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset','grppol_prs_many','policy_rules',rule_uuid_list)!=0:
           self._log.info("# Step 3A: Verify Policy RuleSet and its Multiple PRs, == Failed")
           return 0
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset',ruleset_uuid,name='grppol_prs_many',\
                                                description='For devstack demo',shared='False')==0:
           self._log.info("# Step 3B: Verify Policy RuleSet and its shared=False, == Failed")
           return 0
    ...
    def test_gbp_prs_pr_shared_func_2(self):
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2,'ruleset','grppol_prs_many',shared='True') == 0:
           self._log.info("# Step 3: Updating Policy RuleSet's Attribute shared=True , Failed" )
           return 0
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset','grppol_prs_many','policy_rules',rule_uuid_list)!=0:
           self._log.info("# Step 3A: Verify Policy RuleSet and its Multiple PRs using -show option == Failed")
           return 0
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset',ruleset_uuid,name='grppol_prs_many',\
                                                description='For devstack demo',shared='True')==0:
           self._log.info("# Step 3B: Verify Policy RuleSet and its shared=True, == Failed")
           return 0
