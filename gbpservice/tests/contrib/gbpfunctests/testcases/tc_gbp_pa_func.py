...
def main():
    ...
    report_results('test_gbp_pa_func','test_results.txt')
    sys.exit(1)
...
class test_gbp_pa_func(object):
	...
	self._log.info('\n## Step 1: Create Action with default attrib values##\n')
        ...
        if self.gbpcfg.gbp_action_config(0,self.act_name) == 0:
            ...
            return 0
        if self.gbpverify.gbp_action_verify(0,self.act_name,act_uuid) != 0:
            self._log.info("\n## Step 3A: Verify Action is Deleted using -list option == Failed")
            return 0
        if self.gbpverify.gbp_action_verify(1,self.act_name,id=act_uuid,action_type='allow',shared='False') != 0:
        ...
        act_uuid = self.gbpcfg.gbp_action_config(1,self.act_name)
        if act_uuid:
           self._log.info("Step 4: Re-created a Policy Action with default inorder to delete with ID")
           self._log.info('\n## Step 5: Delete Action using UUID ##\n')
           if self.gbpcfg.gbp_action_config(0,act_uuid) == 0:
               self._log.info("\n## Step 5: Delete Action using UUID == Failed") 
               return 0
           if self.gbpverify.gbp_action_verify(0,act_uuid,self.act_name)!=0:
              self._log.info("\n## Step 5A: Verify Action is Deleted using -list option == Failed")
              return 0
           if self.gbpverify.gbp_action_verify(1,act_uuid,name=self.act_name,action_type='allow',shared='False') !=0:
              self._log.info("\n## Step 5B: Verify Action is Deleted using -show option == Failed")
              return 0
           self._log.info("\n## Step 5: Delete of Policy Action using UUID == Passed")
        else:
           self._log.info("\n## Step 6: Recreate of Policy Action using Default == Failed")
           return 0             
        if rep_cr==0 and rep_del==0:
    ...
    def test_upd_ver_del(self):
        ...
        if act_uuid == 0:
            self._log.info("## Step 1: Create Action == Failed")
            return 0
        self._log.info("\n## Step 1A: Creating a Service Chain Spec to be used for UPdating Polic Action")
        spec_cr_cmd='gbp servicechain-spec-create demo_spec | grep id | head -1'
