...
class test_gbp_pr_pc_pa_shared_func(object):
    ...
    def test_gbp_pr_pc_pa_shared_func_1(self):
        ...
        obj_uuid = {}
        self.cls_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'classifier',self.cls_name,shared=True)
        if self.cls_uuid == 0:
        ...
        self.act_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'action',self.act_name,shared=True)
        if self.act_uuid == 0:
           self._log.info("\n Creation of Policy Action with shared=True, Failed\n")
           return 0
        obj_uuid['action']=self.act_uuid
        self._log.info("\n## Step 1C: Create Policy Rule with PC & PA with shared=True ##")
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'rule',self.rule_name,classifier=self.cls_uuid,action=self.act_uuid,shared=True)
        if rule_uuid == 0:
           self._log.info("# Step 1: Creation of Policy Rule with shared=True, Failed")
           return 0
        ## Verify PA,PC,PR
        self._log.info('## Step 2: Verify Policy Rule, Policy Classifier, Policy Action with shared=True')
        for obj,uuid in obj_uuid.iteritems():
            if self.gbpverify.gbp_policy_verify_all(1,obj,uuid,shared=True) == 0:
               self._log.info("# Step 2A_%s: Verify Policy %s.upper() with shared=True, Failed" %(obj,obj))
               return 0
        if self.gbpverify.gbp_policy_verify_all(1,'rule',self.rule_name,id=rule_uuid,\
                                                ...
                                                policy_actions=self.act_uuid, shared=True) == 0:
           self._log.info("# Step 2B: Verify Policy Rule with shared=True, Failed")
           return 0
        ## Update PR
        self._log.info("\n## Update the Policy Rule with shared=False ##\n")
        if self.gbpcfg.gbp_policy_cfg_all(2,'rule',rule_uuid,shared=False) == 0:
           self._log.info("\nStep 3: Updating Policy Rule's shared=False, Failed")
           return 0
        ## Verify the PR after update
        if self.gbpverify.gbp_policy_verify_all(1,'rule',rule_uuid,shared=False) == 0:
           self._log.info("# Step 4: Verify Policy Rule with shared=False, Failed")
           return 0
    ...
    def test_gbp_pr_pc_pa_shared_func_2(self):
        ...
        obj_uuid_false = {}
        self.cls_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'classifier','cls_shared')
        if self.cls_uuid == 0:
        ...
        self.act_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'action','act_shared')
        if self.act_uuid == 0:
        ...
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'rule','pr_true',classifier=obj_uuid_true['classifier'],action=obj_uuid_true['action'],shared=True)
        if rule_uuid == 0:
           self._log.info("# Step 3: Creation of Policy Rule with shared=True using attributes PA+PC(shared=True), Failed")
           return 0
        ## Update and Verify the PR(shared=True) with PA+PC(shared=False)
        self._log.info("\n## Step 4A: Update the Policy Rule with PC & PA which are with shared=False ##")
        if self.gbpcfg.gbp_policy_cfg_all(1,'rule',rule_uuid,classifier=obj_uuid_false['classifier'],action=obj_uuid_false['action']) != 0:
           self._log.info("# Step 4A: Updating Policy Rule(shared=True) by attributes PA+PC(shared=False) DID NOT Fail")
           #return 0  ##<< we have a bug for this
        self._log.info("\n## Step 4B: Verify the Policy Rule initial attributes PA,PC,shared=True ##")
        if self.gbpverify.gbp_policy_verify_all(1,'rule','pr_true',id=rule_uuid,\
                                                policy_classifier_id=obj_uuid_true['classifier'],enabled='True',\
                                                policy_actions=obj_uuid_true['action'], shared=True) == 0:
           self._log.info("# Step 4B: Verify Policy Rule with shared=True, Failed")
           return 0
        ## Update and Verify the PR(shared=False) with PA+PC(shared=False)
        self._log.info("\n## Step 5A: Update the Policy Rule's shared=False  along with PC+PA(shared=False) ##")
        if self.gbpcfg.gbp_policy_cfg_all(1,'rule',rule_uuid,classifier=obj_uuid_false['classifier'],action=obj_uuid_false['action'],shared=False) == 0:
           self._log.info("# Step 5A: Updating Policy Rule(shared=False) by attributes PA+PC(shared=False), Failed")
           return 0
        self._log.info("\n## Step 5B: Verify the Policy Rule attributes PA,PC,shared=False ##")
        if self.gbpverify.gbp_policy_verify_all(1,'rule','pr_true',id=rule_uuid,\
                                                policy_classifier_id=obj_uuid_false['classifier'],enabled='True',\
                                                policy_actions=obj_uuid_false['action'], shared=False) == 0:
           self._log.info("# Step 5B: Verify Policy Rule with shared=False, Failed")
           return 0
    ...
    def test_gbp_pr_pc_pa_shared_func_3(self):
        ...
        self._log.info('\n## Step 1A: Create a PC with shared=True ##')
        obj_uuid = {}
        self.cls_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'classifier','cls_true',shared=True)
        if self.cls_uuid == 0:
           self._log.info("\n Creation of Policy Classifier with shared=False, Failed\n")
           return 0
        obj_uuid['classifier']=self.cls_uuid
        ...
        self._log.info("\n## Step 2A: Create Policy Rule(shared=True) with PA(shared=False) & PC(shared=True) ##")
        if self.gbpcfg.gbp_policy_cfg_all(1,'rule','true_pr',classifier=obj_uuid['classifier'],action=obj_uuid['action'],shared=True) !=0:
           self._log.info("# Step 2A: Creation of Policy Rule with shared=True using attributes PA(shared=False)+PC(shared=True) did NOT Fail")
           return 0
        self._log.info("\n## Step 2B: Create Policy Rule(shared=False) with PA(shared=False) & PC(shared=True)& Verify ##")
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'rule','true_pr',classifier=obj_uuid['classifier'],action=obj_uuid['action'])
        if rule_uuid == 0:
           self._log.info("# Step 2B: Creation of Policy Rule with shared=False(default) using attributes PA(shared=False)+PC(shared=True), Failed")
           return 0
        if self.gbpverify.gbp_policy_verify_all(1,'rule',rule_uuid,\
                                                policy_classifier_id=obj_uuid['classifier'],enabled='True',\
                                                policy_actions=obj_uuid['action'],shared=False) == 0:
           self._log.info("# Step 2C: Verify Policy Rule with shared=False, Failed")
           return 0
        ## Update the Policy Action with shared=True
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2,'action',self.act_uuid,shared=True) == 0:
           ...
           return 0
        ## Create and Verify a PR with shared=True using above PA+PC(shared=True)
        self._log.info("\n## Step 4: Create and Verify a PR with shared=True using above PA+PC(shared=True)\n")
        true_rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'rule','true_pr',classifier=obj_uuid['classifier'],action=obj_uuid['action'],shared=True)
        if true_rule_uuid == 0:
           self._log.info("# Step : Creation of Policy Rule with shared=True using attributes PA+PC(shared=True), Failed")
           return 0
        if self.gbpverify.gbp_policy_verify_all(1,'rule',true_rule_uuid,\
                                                policy_classifier_id=obj_uuid['classifier'],enabled='True',\
                                                policy_actions=obj_uuid['action'],shared=True) == 0:
           self._log.info("# Step 4B: Verify Policy Rule with shared=True, Failed")
           return 0
