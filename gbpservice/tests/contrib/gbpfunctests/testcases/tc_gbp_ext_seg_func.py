...
class test_gbp_prs_func(object):
    ...
    def test_gbp_prs_func_1(self,name_uuid='',ruleset_uuid='',rep_cr=0,rep_del=0):
	 ...
	 self._log.info('\n## Step 1: Create RuleSet with default attrib vals##\n')
         ruleset_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'ruleset',name_uuid)
         if ruleset_uuid == 0:
	    self._log.info("# Step 1: Create RuleSet == Failed")
            return 0
         self._log.info('# Step 2A: Verify RuleSet using -list cmd')
         if self.gbpverify.gbp_policy_verify_all(0,'ruleset',name_uuid,ruleset_uuid) == 0: 
            self._log.info("# Step 2A: Verify RuleSet using -list option == Failed")
            return 0
         self._log.info('# Step 2B: Verify RuleSet using -show cmd')
         if self.gbpverify.gbp_policy_verify_all(1,'ruleset',name_uuid,id=ruleset_uuid,shared='False') == 0:
            self._log.info("# Step 2B: Verify RuleSet using -show option == Failed")
            return 0
        ####### 
        if rep_del == 0 or rep_del == 1: 
         ...
         if self.gbpcfg.gbp_policy_cfg_all(0,'ruleset',name_uuid) == 0:
            ...
            return 0
         if self.gbpverify.gbp_policy_verify_all(0,'ruleset',name_uuid,ruleset_uuid) != 0:
            self._log.info("# Step 3A: Verify RuleSet is Deleted using -list option == Failed")
            return 0
         if self.gbpverify.gbp_policy_verify_all(1,'ruleset',name_uuid,id=ruleset_uuid,shared='False') != 0:
            self._log.info("# Step 3B: Verify RuleSet is Deleted using -show option == Failed")
            return 0
         if rep_cr==0 and rep_del==0:
    ...
    def test_gbp_prs_func_2(self):
        ...
        if ruleset_uuid == 0:
           ...
           return 0
        self._log.info('# Step 2A: Verify RuleSet using -list cmd')
        if self.gbpverify.gbp_policy_verify_all(0,'ruleset',self.ruleset_name,ruleset_uuid) == 0:
           self._log.info("# Step 2A: Verify RuleSet using -list option == Failed")
           return 0
        self._log.info('# Step 2B: Verify RuleSet using -show cmd')
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset',self.ruleset_name,id=ruleset_uuid,\
                                                policy_rules=self.rule_uuid,shared='False') == 0:
           self._log.info("# Step 2B: Verify RuleSet using -show option == Failed")
           return 0
    ...
    def test_gbp_prs_func_3(self):
        ...
        ruleset_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'ruleset',self.ruleset_name,policy_rules=self.rule_name)
        if ruleset_uuid != 0:
            self._log.info("Step 1: Create RuleSet Passed, UUID == %s\n" %(ruleset_uuid))
        else:
            self._log.info("# Step 1: Create RuleSet == Failed")
            return 0
    ...
    def test_gbp_prs_func_5(self):
        ...
        if child_uuid == 0:
           ...
           return 0
        parent_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'ruleset','demo_par_prs',policy_rules=self.rule_name,child_policy_rule_sets=child_uuid)
        if parent_uuid == 0:
           self._log.info("\n## Step 2: Create Parent Policy RuleSet == Failed")
           return 0
        self._log.info('# Step 2A: Verify Parent and Child Policy RuleSet using -show cmd')
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset','demo_par_prs',id=parent_uuid,\
                                                policy_rules=self.rule_uuid,shared='False',\
                                                child_policy_rule_sets=child_uuid) == 0:
           self._log.info("\n## Step 2A: Verify Parent RuleSet using -show option == Failed")
           return 0
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset','demo_child_prs',id=child_uuid,\
                                                policy_rules=self.rule_uuid,shared='False',\
        ...
        if child_uuid == 0:
           ...
           return 0
        if self.gbpcfg.gbp_policy_cfg_all(2,'ruleset','demo_par_prs',child_policy_rule_sets=child_uuid)==0:
           self._log.info("# Step 5: Associating Child PRS by Updating Parent PRS == Failed")
           return 0
        self._log.info('# Step 5A: Verify Parent and Child Policy RuleSet using -show cmd')
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset','demo_par_prs',id=parent_uuid,\
                                                policy_rules=self.rule_uuid,shared='False',\
                                                child_policy_rule_sets=child_uuid) == 0:
           self._log.info("# Step 5A: Verify Parent RuleSet using -show option == Failed")
           return 0
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset','demo_child_prs',id=child_uuid,\
                                                policy_rules=self.rule_uuid,shared='False',\
                                                parent_id=parent_uuid) == 0:
           self._log.info("# Step 5B: Verify Parent RuleSet using -show option == Failed")
           return 0
    ...
    def test_gbp_prs_func_6(self):
        ...
        ###### Testcase work-flow starts
        self._log.info("\n## Step 1: Create 4 Policy RuleSets, 2 Parent & 2 Child with PR ##")
        ch_uuids,par_uuids=[],[]
        for i in range(1,3):
          child_name, par_name='child_%s_prs' %(i),'par_%s_prs' %(i)
          child_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'ruleset',child_name,policy_rules=self.rule_name)
          if child_uuid == 0:
             self._log.info("\n## Step 1: Create Child Policy RuleSet == Failed")
             return 0
          parent_uuid=self.gbpcfg.gbp_policy_cfg_all(1,'ruleset',par_name,policy_rules=self.rule_name,child_policy_rule_sets=child_uuid)
          if parent_uuid == 0:
             self._log.info("\n## Step 2: Create Parent Policy RuleSet == Failed")
             return 0
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2,'ruleset','par_1_prs',child_policy_rule_sets='"%s %s"' %(ch_uuids[0],ch_uuids[1]))==0:
           self._log.info("\n## Step 3: Update Parent Policy RuleSet == Failed") 
           return 0
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset','par_1_prs','child_policy_rule_sets',[ch_uuids[0],ch_uuids[1]]) ==0:
           self._log.info("\n## Step 3A: Child PRS NOT Found in Parent PRS")
           return 0
        ## Delete Child PRSs
        for i in range(2):
            if self.gbpcfg.gbp_policy_cfg_all(0,'ruleset',ch_uuids[i]) == 0:
               self._log.info("\n## Step 5: Delete of Child PRS child_%s_prs" %(i))
               return 0
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset','par_1_prs','child_policy_rule_sets',[ch_uuids[0],ch_uuids[1]])!=0:
           self._log.info("\n## Step 5A: Stale Child PRS Mapping still persists in Parent PRS")
           return 0
