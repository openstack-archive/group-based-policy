...
def main():
    ...
    test.cleanup()
    report_results('test_gbp_pr_neg','test_results.txt')
    sys.exit(1)
...
class test_gbp_pr_neg(object):
	...
	self._log.info("\n## Step 1: Create Rule with Invalid PC##\n")
        if self.gbpcfg.gbp_policy_cfg_all(1,'rule',self.rule_name,classifier="INVALID")!=0:
	   self._log.info("\n## Step 1: Create Policy Rule with Invalid Policy Classifier did NOT Fail")
           return 0
        self._log.info("\n## Step 1A: Verify Policy Rule has been rolled back")
        if self.gbpverify.gbp_policy_verify_all(1,'rule',self.rule_name) != 0:
    ...
    def test_gbp_pr_neg_2(self):
        ...
        if self.gbpcfg.gbp_policy_cfg_all(1,'rule',self.rule_name,classifier=self.cls_name,action='INVALID')!=0:
    ...
    def test_gbp_pr_neg_3(self):
        ...
        for attr_val in attrib_list:
          if self.gbpcfg.gbp_policy_cfg_upd_all('rule',rule_uuid,attr_val) !=0:
             self._log.info("\nStep 2: Updating Policy Rule's Attribute %s with Invalid Value did NOT Fail" %(attr_val))
             return 0
        ## Verify starts
        if self.gbpverify.gbp_policy_verify_all(1,'rule',rule_uuid,name=self.rule_name,\
                                                policy_classifier_id=self.cls_uuid,\
                                                policy_actions=self.act_uuid)==0:
             self._log.info("# Step 2B: Verify Policy Rule Updated did NOT roll back")
             return 0
