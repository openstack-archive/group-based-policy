...
def main():
    ...
    test.cleanup()
    report_results('test_gbp_pc_func','test_results.txt')
    sys.exit(1)
...
class test_gbp_pc_func(object):
	...
	self._log.info("\n## Step 1: Create Classifier with default attrib vals##\n")
        cls_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'classifier',self.cls_name)
        if cls_uuid != 0:
        ...
        if self.gbpverify.gbp_classif_verify(1,self.cls_name,id=cls_uuid)==0:
        ...
        if self.gbpcfg.gbp_policy_cfg_all(0,'classifier',self.cls_name):
            self._log.info("\n## Step 3: Delete Classifier using Name == %s, Passed" %(self.cls_name))
        else:
            self._log.info("\n## Step 3: Delete Classifier using Name == Failed")
            return 0
        if self.gbpverify.gbp_classif_verify(0,self.cls_name,cls_uuid)!=0:
            self._log.info("\n## Step 3A: Verify Classifier is Deleted using -list option == Failed")
            return 0
        if self.gbpverify.gbp_classif_verify(1,self.cls_name,id=cls_uuid,shared='False')!=0:
            self._log.info("\n## Step 3B: Verify Classifier is Deleted using -show option == Failed")
            return 0

        cls_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'classifier',self.cls_name)
        if cls_uuid:
           self._log.info("\n## Step 4: Re-created a Policy Classifier with default inorder to delete with ID")
           self._log.info("\n## Step 5: Delete Classifier using UUID  ##")
           if self.gbpcfg.gbp_policy_cfg_all(0,'classifier',cls_uuid):
               self._log.info("\n## Step 5: Delete Classifier Passed using UUID == %s" %(cls_uuid))
           else:
               self._log.info("\n##  Step 5: Delete Classifier using UUID == Failed") 
           if self.gbpverify.gbp_classif_verify(0,self.cls_name,cls_uuid)!=0:
              self._log.info("\n##  Step 5A: Verify Classifier is Deleted using -list option == Failed")
              return 0
           if self.gbpverify.gbp_classif_verify(1,self.cls_name,id=cls_uuid,shared='False')!=0:
              self._log.info("\n## Step 5B: Verify Classifier is Deleted using -show option == Failed")
              return 0
           self._log.info("\n## Step 5: Delete of Policy Classifier using UUID == Passed")
        else:
           self._log.info("\n## Step 6: Recreate of Policy Classifier using Default == Failed")
           return 0             
    ...
    def test_gbp_pc_func_2(self):
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2,'classifier',cls_uuid,protocol='tcp')==0:
             self._log.info("\n## Step 2: Updating Policy Classifier's attribute Protocol, Failed")
             return 0       
        if self.gbpcfg.gbp_policy_cfg_all(2,'classifier',cls_uuid,direction='bi')==0:
             self._log.info("\n## Step 2: Updating Policy Classifier's attribute Direction, Failed")
             return 0
    ...
    def test_gbp_pc_func_4(self):
        ...
        if cls_uuid == 0:
            ...
            return 0
        if self.gbpverify.gbp_classif_verify(1,self.cls_name,id=cls_uuid,protocol='tcp',direction='bi',\
                                            port_range='22:1022',description='For devstack demo') == 0:
           self._log.info("\n## Step 1B: Verify Policy Classifier Attributes using -show option == Failed")
           return 0
