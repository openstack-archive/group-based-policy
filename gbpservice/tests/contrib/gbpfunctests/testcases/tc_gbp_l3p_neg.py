...
def main():
    ...
    test.cleanup()
    report_results('test_gbp_l3p_neg','test_results.txt')
    sys.exit(1)
...
class test_gbp_l3p_neg(object):
    ...
    def test_gbp_l3p_neg_1(self):
        ...
        count=0
        invalid_pools=['x.y.0.0/24','0.2323.0.0/24','0.0.0.0/0','255.255.255.255/32']
        for pool in invalid_pools:
          self._log.info("\n## Step 1A: Create L3Policy with Invalid IP-Pool = %s ##" %(pool))
          if self.gbpcfg.gbp_policy_cfg_all(1,'l3p',self.l3p_name,ip_pool=pool) != 0:
             self._log.info("# Step 1A: Create L3Policy with Invalid IP-Pool %s did NOT fail" %(pool))
          self._log.info('# Step 1A: Verify L3Policy did NOT get created')
          if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',self.l3p_name) !=0:
             self._log.info("# Step 1A: L3Policy did NOT fail to create even with Invalid IP-Pool %s" %(pool))
             count+=1
        if count > 0:
             return 0
        else:
    ...
    def test_gbp_l3p_neg_2(self):
        ...
        for prefix in invalid_prefix_length:
          self._log.info("\n## Step 1A: Create L3Policy with Invalid Prefix-lenght = %s ##" %(prefix))
          if self.gbpcfg.gbp_policy_cfg_all(1,'l3p',self.l3p_name,subnet_prefix_length=prefix) != 0:
             self._log.info("# Step 1A: Create L3Policy with Invalid IP-Pool %s did NOT fail" %(prefix))
          self._log.info('# Step 1A: Verify L3Policy did NOT get created')
          if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',self.l3p_name) !=0:
             self._log.info("# Step 1A: L3Policy did NOT fail to create even with Invalid IP-Pool %s" %(prefix))
             cnt+=1
        if cnt > 0:
           return 0
        else:
    ...
    def test_gbp_l3p_neg_3(self):
        ...
        for ip,pref in mix_attr.iteritems():
          self._log.info("\n## Step 1A: Create L3Policy with IP-Pool = %s & Subnet-Pref-Len = %s ##" %(ip,pref))
          if self.gbpcfg.gbp_policy_cfg_all(1,'l3p',self.l3p_name,ip_pool=ip,subnet_prefix_length=pref) != 0:
             self._log.info("# Step 1A: Create L3Policy with mix of valid and invalid did NOT fail")
          self._log.info('# Step 1A: Verify L3Policy did NOT get created')
          if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',self.l3p_name) !=0:
             self._log.info("# Step 1A: L3Policy did NOT fail to create even with mix of Valid and Invalid attrs %s")
             _pass+=1
        if _pass > 0:
           return 0
        else:
    ...
    def test_gbp_l3p_neg_4(self):
        ...
        if l3p_uuid == 0:
            ...
            return 0
        if self.gbpcfg.gbp_policy_cfg_all(2,'l3p',l3p_uuid,subnet_prefix_length='32') != 0:
           self._log.info("\n## Step 2: Updating L3Policy's Subnet-Prefix-Length with Invalid Value=32 did NOT fail")
           return 0
        if self.gbpcfg.gbp_policy_cfg_all(2,'l3p',l3p_uuid,ip_pool='20.20.0.0/24') != 0:
           self._log.info("\n## Step 3: Updating L3Policy's Immutable attr IP-Pool did NOT fail")
           return 0
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,\
                                                  id=l3p_uuid,name=self.l3p_name,ip_pool='10.0.0.0/8',\
                                                  subnet_prefix_length='24') == 0:
           self._log.info("\n## Step 4: L3Policy config did NOT roll back to original default values")
           return 0
        self.gbpcfg.gbp_policy_cfg_all(0,'l3p',l3p_uuid) #clean-up before next testcase
    ...
    def test_gbp_l3p_neg_5(self):
        ...
        if l3p_uuid == 0:
            self._log.info("\n## Step 1: Create L3Policy == Failed")
            return 0
        if self.gbpcfg.gbp_policy_cfg_all(2,'l3p',l3p_uuid,subnet_prefix_length='4') != 0:
           self._log.info("\n## Step 2: Updating L3Policy's Subnet-Prefix-Length > default Mask-length(8) did NOT fail")
           return 0
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,\
                                                  id=l3p_uuid,name=self.l3p_name,ip_pool='10.0.0.0/8',\
                                                  subnet_prefix_length='24') == 0:
           self._log.info("\n## Step 3: L3Policy config did NOT roll back to original default values")
           return 0        
        if self.gbpcfg.gbp_policy_cfg_all(1,'l3p','new_l3p',ip_pool='20.20.20.0/24',subnet_prefix_length='16') != 0:
           self._log.info("\n## Step 4: Creating L3Policy with Subnet-Prefix-Length > Mask-Length(24) did NOT fail")
           return 0
