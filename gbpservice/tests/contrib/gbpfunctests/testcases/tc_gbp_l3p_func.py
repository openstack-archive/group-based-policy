...
class test_gbp_l3p_func(object):
    ...
    def test_gbp_l3p_func_1(self,name_uuid='',l3p_uuid='',rep_cr=0,rep_del=0):
	 ...
	 self._log.info('\n## Step 1: Create L3Policy with default attrib vals##\n')
	    ...
	    self._log.info("\n## Step 1: Create L3Policy == Failed")
         ...
         if 'Ubuntu' in platform.linux_distribution(): #Only for devstack, not for GBP-APIC
          rtr_uuid=self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,ret='default',\
                                                         id=l3p_uuid,name=name_uuid,ip_pool='10.0.0.0/8',\
                                                         subnet_prefix_length='24',ip_version='4')
          if rtr_uuid!=0 and isinstance(rtr_uuid,str):
            ...
            if self.gbpverify.neut_ver_all('router',rtr_uuid,name=rtr_name,admin_state_up='True',status='ACTIVE')==0:
               self._log.info("\n## Step 2D: Verify L3Policy using -show option == Failed")
               return 0
          else:
            self._log.info("\n## Step 2C: Verify L3Policy using -show option == Failed")
            return 0
         else:
            if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,id=l3p_uuid,name=name_uuid,\
        ...
        if rep_del == 0 or rep_del == 1: 
         ...
         if self.gbpcfg.gbp_policy_cfg_all(0,'l3p',name_uuid) == 0:
            ...
            return 0
         if self.gbpverify.gbp_l2l3ntk_pol_ver_all(0,'l3p',name_uuid,l3p_uuid) != 0:
            self._log.info("\n## Step 3A: Verify L3Policy is Deleted using -list option == Failed")
            return 0
         if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',name_uuid,l3p_uuid) != 0:
            self._log.info("\n## Step 3B: Verify L3Policy is Deleted using -show option == Failed")
            return 0
         if rep_cr==0 and rep_del==0:
    ...
    def test_gbp_l3p_func_2(self):
        ...
        if 'Ubuntu' in platform.linux_distribution(): #Only for devstack, not for GBP-APIC
          rtr_uuid=self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,ret='default',\
                                                         id=l3p_uuid,name=self.l3p_name,ip_pool='20.20.0.0/24',\
                                                         subnet_prefix_length='28',ip_version='4')
          if rtr_uuid!=0 and isinstance(rtr_uuid,str):
            ...
            if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,id=l3p_uuid,name=self.l3p_name,\
                                                      ip_pool='20.20.0.0/24',subnet_prefix_length='28',\
                                                      ip_version='4')==0:
               self._log.info("\n## Step 2C: Verify L3Policy using -show option == Failed")

        if self.gbpcfg.gbp_policy_cfg_all(2,'l3p',self.l3p_name,subnet_prefix_length='26') == 0:
           self._log.info("\n## Step 3: UPdating L3Policy attributes == Failed")
           return 0
        self._log.info("\n## Step 3: Verify that Updated Attributes in L3Policy")
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',self.l3p_name,\
                                                         id=l3p_uuid,name=self.l3p_name,ip_pool='20.20.0.0/24',\
                                                         subnet_prefix_length='26',ip_version='4')==0:
    ...
    def test_gbp_l3p_func_3(self):
        ...
        ## Associating L2Policy with non-default L3Policy(should Fail) and UPdating the L3Policy(in-use/default)
        if self.gbpcfg.gbp_policy_cfg_all(2,'l2p',self.l2p_name,l3_policy_id=l3p_uuid) != 0:
           self._log.info("\n## Updating/Changing L3Policy attribute of L2Policy did NOT Fail")
           return 0
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l2p',self.l2p_name,l3_policy_id=def_l3p_uuid) == 0:
           self._log.info("\n## Step 3A: Verify L2Policy is still associated to its  default L3Policy == Failed")
           return 0
        if self.gbpcfg.gbp_policy_cfg_all(2,'l3p',def_l3p_uuid,subnet_prefix_length='27') == 0:
           self._log.info("\n## Step 4: UPdating default L3Policy's attributes == Failed")
           return 0
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',def_l3p_uuid,\
                                                  id=def_l3p_uuid,ip_pool='10.0.0.0/8',\
                                                  l2_policies=l2p_uuid,subnet_prefix_length='27',ip_version='4')==0:
           self._log.info("\n## Step 4A: Verify L3Policy after associating to the L2Policy == Failed")
           return 0
        ...
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',def_l3p_uuid)!=0:
           self._log.info("\n## Step 5B: Verify Auto-Delete of default L3Policy == Failed")
           return 0
    ...
    def test_gbp_l3p_func_4(self):
        ...
        self._log.info("\n## Step 1: Create Policy L3Policy with non-default attrs and values ")
        l3p_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'l3p',self.l3p_name,ip_pool='40.50.0.0/24',subnet_prefix_length='25')
        if l3p_uuid == 0:
           self._log.info("\n## Step 1: Create L3Policy == Failed")
           return 0
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',l3p_uuid,\
                                                  id=l3p_uuid,name=self.l3p_name,ip_pool='40.50.0.0/24',\
                                                  subnet_prefix_length='25',ip_version='4')==0:
           self._log.info("\n## Step 1A: Verify non-default == Failed")
           return 0
        ## Create and verify multiple L2 policy with above non-default L3P
        self._log.info("\n## Step 2: Create and Verify multiple(n=10) L2Policy associated with 1 non-default L3P")
        l2p_uuid_list=[]
        n,i=11,1
        while i < n:
            ...
            if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l2p',l2p_name,id=l2p_uuid,l3_policy_id=l3p_uuid) == 0:
               self._log.info("\n## Step 2D: Verify L2Policy using non-default L3P == Failed")
               return 0
            i+=1
        ## Verify that non-default L3P has all the above create L2Ps
        if self.gbpverify.gbp_obj_ver_attr_all_values('l3p',l3p_uuid,'l2_policies',l2p_uuid_list) == 0:
