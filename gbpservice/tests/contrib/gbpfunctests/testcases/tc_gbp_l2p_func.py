...
def main():
    ...
    test.cleanup()
    report_results('test_gbp_l2p_func','test_results.txt')
    sys.exit(1)
...
class test_gbp_l2p_func(object):
    ...
    def test_gbp_l2p_func_1(self):
        ...
        ## Create and Verify L2Policy with default attrs(L3Policy & implicit Neutron net obj)
	self._log.info('\n## Step 1: Create L2Policy with default attrib vals##\n')
        ...
        self._log.info("\n## Step 2: Verify L2Policy, default L3Policy, and Implicit Neutron objs") 
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(0,'l2p',l2p_uuid,self.l2p_name) == 0: 
        ...
        net_uuid=self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l2p',l2p_uuid,ret='default',\
                                                         id=l2p_uuid,name=self.l2p_name,l3_policy_id=def_l3p_uuid)[1]
        if net_uuid!=0 and isinstance(net_uuid,str):
            if self.gbpverify.neut_ver_all('net',net_uuid,name='l2p_%s' %(self.l2p_name),admin_state_up='True',status='ACTIVE', shared='False')==0:
               self._log.info("\n## Step 2C: Verify implicit neutron network object == Failed")
               return 0
        else:
            self._log.info("\n## Step 2B: Verify L2Policy using -show option == Failed")
            return 0
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l3p',def_l3p_uuid,name='default',\
                                                         id=def_l3p_uuid,l2_policies=l2p_uuid)==0:
            self._log.info("\n## Step 2D: Verify default L3Policy == Failed")
            return 0

        ## Delete the L2Policy and verify L2P,def l3P and Neutron obj are deleted too
        ...
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1,'l2p',l2p_uuid) != 0:
            self._log.info("\n## Step 3A: Verify L2Policy is Deleted using -show option == Failed")
            return 0
        ...
        if self.gbpverify.neut_ver_all('net',net_uuid) != 0:
           self._log.info("\n## Step 3C: Verify Implicit Neutron Network Obj is Deleted == Failed")
           return 0
    ...
    def test_gbp_l2p_func_2(self):
        ...
        if net_uuid!=0 and isinstance(net_uuid,str):
            if self.gbpverify.neut_ver_all('net',net_uuid,name='l2p_%s' %(self.l2p_name),admin_state_up='True',status='ACTIVE',shared='False') == 0:
               self._log.info("\n## Step 2A: Verify implicit neutron network object == Failed")
               return 0
        else:
            self._log.info("\n## Step 2B: Verify L2Policy using -show option == Failed")
            return 0
        ...
        while i < n:
           ...
           uuids = self.gbpcfg.gbp_policy_cfg_all(1,'group',ptg_name,l2_policy=l2p_uuid)
           if uuids != 0:
           ...
           if self.gbpverify.neut_ver_all('subnet',subnet_list[i-1],network_id=net_uuid)== 0:
              ...
              return 0
           i+=1
        if self.gbpverify.gbp_obj_ver_attr_all_values('l2p',l2p_uuid,'policy_target_groups',ptg_list) == 0:
