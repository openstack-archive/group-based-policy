...
def main():
    ...
    test.cleanup()
    report_results('test_gbp_prs_neg','test_results.txt')
    sys.exit(1)
...
class test_gbp_prs_neg(object):
    ...
    def test_gbp_prs_neg_1(self):
        ...
        if self.gbpcfg.gbp_policy_cfg_all(1,'ruleset',self.ruleset_name,policy_rules='INVALID') != 0:
           self._log.info("# Step 1: Create RuleSet with Invalid PR did NOT Fail")
           return 0
        self._log.info('# Step 1A: Verify Policy RuleSet config rolled back')
        if self.gbpverify.gbp_policy_verify_all(1,'ruleset',self.ruleset_name) !=0:
           self._log.info("# Step 1A: Verify RuleSet config roll back did NOT Fail")
           return 0
    ...
    def test_gbp_prs_neg_3(self):
        ...
        ruleset_uuid = self.gbpcfg.gbp_policy_cfg_all(1,'ruleset',self.ruleset_name)
        if ruleset_uuid == 0:
            self._log.info("\n## Step 1: Create RuleSet with default attr == Failed")
            return 0
        self._log.info("\n## Step 2: Update the PRS with VALID PR and INVALID PR")
        if self.gbpcfg.gbp_policy_cfg_all(2,'ruleset',ruleset_uuid,\
                                          policy_rule='"%s INVALID"' %(self.rule_name)) != 0:
           self._log.info("\n## Step 2: Updating Policy RuleSet with VALID and INVALID Policy Rules did NOT Fail")
           return 0
        self._log.info('# Step 2A: Verify RuleSet config update has been rolled back')
