...
validate_loadbalancer_resources() {
    ...
    gbp policy-target-create --policy-target-group fw_lb-provider provider_pt1 | grep ' id ' | awk '{print $4}'
    sleep 5
    ...
    gbp policy-target-create --policy-target-group fw_lb-provider provider_pt2 | grep ' id ' | awk '{print $4}'
    sleep 5
    LBMemberCount=`neutron lb-member-list -f value | wc -l`
...
update_gbp_resources() {
    ...
    gbp servicechain-node-update --template-file $TOP_DIR/nfp-templates/fw_updated_template.yml FW_LB-FWNODE
    ...
    gbp group-create fw_lb-consumer --consumed-policy-rule-sets "lb-webredirect-ruleset=None"
