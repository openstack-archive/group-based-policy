...
PORT1=$(gbp policy-target-create port2-pt --policy-target-group limited | awk "/port_id/ {print \$4}")
...
PORT2_VM_UUID=`nova boot --flavor m1.tiny --image $DEFAULT_IMAGE_NAME --nic port-id=$PORT2 port1-vm | grep ' id ' | cut -d"|" -f3 | sed 's/ //g'`
