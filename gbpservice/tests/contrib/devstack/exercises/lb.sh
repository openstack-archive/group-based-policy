...
validate_loadbalancer_resources() {
    ...
    gbp policy-target-create --policy-target-group lb-provider provider_pt1 | grep ' id ' | awk '{print $4}'
    sleep 5
    ...
    gbp policy-target-create --policy-target-group lb-provider provider_pt2 | grep ' id ' | awk '{print $4}'
    sleep 5
    LBMemberCount=`neutron lb-member-list -f value | wc -l`
