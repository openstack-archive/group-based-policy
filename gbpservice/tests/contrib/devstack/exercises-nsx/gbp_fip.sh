...
if [ -z "$EXT_NET_ID" ] ; then
    ...
    EXT_SUBNET_ID=$(neutron subnet-create --ip_version 4 --gateway 172.16.73.1 --name public-subnet $EXT_NET_ID 172.16.73.0/24 --enable-dhcp=False | grep ' id ' | get_field 2)
...
EXT_SEGMENT_ID=$(gbp external-segment-create --ip-version 4 --external-route destination=20.0.0.0/0,nexthop=$EXT_SUBNET_GW --shared True --subnet_id=$EXT_SUBNET_ID  --cidr $EXT_SUBNET_CIDR default | grep ' id ' | awk '{print $4}' )
