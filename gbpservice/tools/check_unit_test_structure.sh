...
gbpservice_path=$(cd "$(dirname "$0")/.." && pwd)
base_test_path=gbpservice/tests/unit
test_path=$gbpservice_path/$base_test_path
...
for test_file in ${test_files[@]}; do
    ...
    expected_path=$(dirname $gbpservice_path/gbpservice/$relative_path)
