...
class SfcAIMDriver(SfcAIMDriverBase, db.DbMixin):
    ...
    def _map_port_pair_group(self, plugin_context, ppg):
        ...
        dc.physical_domain_name = self._get_ppg_physdom(plugin_context, ppg)
    ...
    def _map_port_chain(self, plugin_context, pc):
        ...
        for flc in self.flowc_plugin.get_flow_classifiers(
            ...
            sg = self._get_pc_service_graph(p_ctx.session, pc, flc)
            ...
            subject = aim_resource.ContractSubject(
                ...
                bi_filters=[self.aim_mech_driver._any_filter_name])
    ...
    def _map_flow_classifier(self, plugin_context, flowc):
        ...
        src_group = self.aim.get(aim_ctx, self._map_flowc_network_group(
            plugin_context, source_net, flowc['source_ip_prefix'], flowc,
            FLOWC_SRC_PREFIX))
        dst_group = self.aim.get(aim_ctx, self._map_flowc_network_group(
        ...
        self.aim.create(aim_ctx, src_group, overwrite=True)
        self.aim.create(aim_ctx, dst_group, overwrite=True)
    ...
    def _delete_flowc_network_group_mapping(self, plugin_context, net, flowc,
        ...
        if l3out:
            ...
            epg = self.aim.get(
                aim_ctx, self._get_epg_by_network_id(plugin_context.session,
                                                     net['id']))
            contract = self._get_flc_contract(plugin_context.session, flowc)
            try:
    ...
    def _get_ppg_physdom(self, plugin_context, ppg):
        pp = self.sfc_plugin.get_port_pair(plugin_context,
                                           ppg['port_pairs'][0])
        net = self._get_network_by_port_id(plugin_context, pp['ingress'])
        aim_ctx = aim_context.AimContext(db_session=plugin_context.session)
        epg = self._get_epg_by_network_id(plugin_context.session, net['id'])
        epg = self.aim.get(aim_ctx, epg)
        if not epg.physical_domains:
            raise exceptions.NoPhysicalDomainSpecifiedInServiceEPG(
                epg_id=epg.dn)
        # TODO(ivar): is there a way to pinpoint exactly which domain a given
        # endpoint belongs to?
        return epg.physical_domains[0]['name']

    def _get_pc_service_graph(self, session, pc, flowc):
        tenant_aid = COMMON_TENANT_NAME
