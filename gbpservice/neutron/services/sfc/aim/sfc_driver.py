...
class SfcAIMDriver(base.SfcDriverBase):
    ...
    def _delete_port_chain_mapping(self, plugin_context, pc):
        ...
        for flc_id in pc['flow_classifiers']:
            flc_aid = self.name_mapper.flow_classifier(session, flc_id)
    ...
    def _get_ppg_provider_consumer_bds(self, plugin_context, ppg):
        ...
        for pp in pps:
            ...
            if ingress and egress:
                ingress, egress = ingress[0], egress[0]
                ingress_net = self.plugin.get_network(plugin_context,
                                                      ingress['network_id'])
                egress_net = self.plugin.get_network(plugin_context,
                                                     egress['network_id'])
                ingress_bd, _ = self._map_network(plugin_context.session,
                                                  ingress_net)
                egress_bd, _ = self._map_network(plugin_context.session,
                                                 egress_net)
                # Every port pair will return the same result
                return ingress_bd, egress_bd

    def _map_network(self, session, network):
        # TODO(ivar): share with mechanism driver, it's copied from there
