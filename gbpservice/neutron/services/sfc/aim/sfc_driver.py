...
class SfcAIMDriver(SfcAIMDriverBase):
    ...
    def _validate_port_pair(self, context):
        ...
        ingress_net = self.aim_mech._get_port_network_id(
            context._plugin_context, context.current['ingress'])
        egress_net = self.aim_mech._get_port_network_id(
            context._plugin_context, context.current['egress'])
    ...
    def _validate_port_pair_group(self, context):
        ...
        if port_pairs:
            ...
            if any(x for x in port_pairs[1:] if ([ingress_net, egress_net] != [
                    self.aim_mech._get_port_network_id(context._plugin_context,
                                                       y)
    ...
    def _get_ppg_physdom(self, plugin_context, ppg):
        pp = self.sfc_plugin.get_port_pair(plugin_context,
                                           ppg['port_pairs'][0])
        net = self.aim_mech._get_network_by_port_id(plugin_context,
                                                    pp['ingress'])
