...
class SfcAIMDriver(base.SfcDriverBase):
    ...
    def _map_port_pair_group(self, plugin_context, ppg):
        ...
        ipbr = aim_sg.ServiceRedirectPolicy(tenant_name=dc.tenant_name,
                                            name=pbr_id)
        pbr_id = self.name_mapper.port_pair_group(session, ppg['id'],
                                                  prefix=PBR_EGR_PREFIX)
        epbr = aim_sg.ServiceRedirectPolicy(tenant_name=dc.tenant_name,
                                            name=pbr_id)
    ...
    def _get_ppg_device_cluster(self, session, ppg):
        ...
        return aim_sg.DeviceCluster(tenant_name=tenant_id, name=ppg_id,
                                    display_name=ppg_name, managed=False)

    def _get_flowc_contract(self, session, flowc):
        fc_id = self.name_mapper.flow_classifier(session, flowc['id'])
        fc_name = aim_utils.sanitize_display_name(flowc['name'])
        return aim_sg.DeviceCluster(
            tenant_name=COMMON_TENANT_NAME, name=fc_id, display_name=fc_name,
            managed=False)
