...
class SfcAIMDriver(SfcAIMDriverBase):
    ...
    def _map_flowc_network_group(self, plugin_context, net, cidr, flowc,
        ...
        l3out = self.aim_mech._get_svi_net_l3out(network_db)
        if l3out:
            if cidr.prefixlen == 0:
                # Use default External Network
                ext_net = self.aim_mech._get_default_ext_epg(network_db)
    ...
    def _delete_flowc_network_group_mapping(self, plugin_context, net, flowc,
        ...
        if l3out:
            if cidr.prefixlen != 0:
                ...
                epg = self.aim.get(
                    aim_ctx, self.aim_mech._get_default_ext_epg(network_db))
        else:
