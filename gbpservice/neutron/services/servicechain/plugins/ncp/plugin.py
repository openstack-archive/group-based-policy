...
class NodeCompositionPlugin(servicechain_db.ServiceChainDbPlugin,
    ...
    def create_servicechain_instance(self, context, servicechain_instance):
        ...
        # Actual node deploy
        try:
            self._deploy_servicechain_nodes(context, deployers, instance)
        except Exception:
    ...
    def update_servicechain_instance(self, context, servicechain_instance_id,
        ...
        self._deploy_servicechain_nodes(context, deployers, updated_instance)
    ...
    def update_chains_pt_added(self, context, policy_target):
        ...
        """
        self._update_chains_pt_modifies(context, policy_target, 'added')

    def update_chains_pt_removed(self, context, policy_target):
        """ Auto scaling function.

        Notify the correct set of node drivers that a new policy target has
        been removed to a relevant PTG.
        """
        self._update_chains_pt_modifies(context, policy_target, 'removed')

    def _update_chains_pt_modifies(self, context, policy_target, action):
        ...
        for sci in scis:
            ...
            for update in updaters.values():
                try:
                    ...
                    LOG.error(_("Node Update on policy target addition "
                                "failed, %s"), ex.message)
    ...
    def _deploy_servicechain_nodes(self, context, deployers, instance):
        ...
        for deploy in deployers.values():
            ...
            try:
                ...
                model.set_node_ownership(deploy['context'],
                                         driver.name)
                break
                ...
                with excutils.save_and_reraise_exception():
                    ...
                    driver.destroy(deployers['context'])
