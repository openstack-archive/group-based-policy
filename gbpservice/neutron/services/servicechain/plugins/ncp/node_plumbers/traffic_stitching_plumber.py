...
class TrafficStitchingPlumber(plumber_base.NodePlumberBase):
        ...
        if deployment:
            ...
            for part in deployment:
                ...
                if info['plumbing_type'] == common.PLUMBING_TYPE_ENDPOINT:
                    # No stitching needed, only provider side PTs are created
                    pass
    ...
    def unplug_services(self, context, deployment):
        ...
        for jump_ptg in reversed(jump_ptgs):
            ...
            if l2p['name'].startswith(TSCP_RESOURCE_PREFIX):
                try:
                    self.gbp_plugin.delete_l2_policy(
                        context, jump_ptg['id'])
                except (group_policy.L2PolicyNotFound,
                        group_policy.L2PolicyInUse) as ex:
                    LOG.info(ex.message)
    ...
    def _create_jump_group(self, context, proxied, position, type):
        l2p = self.gbp_plugin.get_l2_policy(context, proxied['l2_policy_id'])
        l3p = self.gbp_plugin.get_l3_policy(context, l2p['l3_policy_id'])

        data = {
            "name": (TSCP_RESOURCE_PREFIX + "l2_" + str(position) + "_" +
                     proxied['name']),
            "description": "Implicitly created stitching l2 policy",
            "l3_policy_id": l3p['id'],
            "network_id": None,
        }
        l2p = self.gbp_plugin.create_l2_policy(context, {'l2_policy': data})
