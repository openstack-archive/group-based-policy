...
class NodeDriverBase(object):
    """Node Centric Chain (NCC) Node Driver base class.

    A Node Driver is the fundamental unit of the NCC service chain plugin.
    ...
    The Node Driver may expose resource needs to the NCC plugin, that will
    ...
    @abc.abstractmethod
    def plumbing_info(self, context):
        """ Tells the NCC Plugin which kind of plumbing is needed by the Node.

        The plumbing info is defined as a collection of needed policy target
        on a specific role, this may vary based on the node
        (got from the NodeDriverContext) that the specific driver is asked to
        deploy. An example of plumbing info is the following:

        {
            "management": <list of PT body updates, one for each needed>,
            "provider": <list of PT body updates, one for each needed>,
            "consumer": <list of PT body updates, one for each needed>
        }
    ...
    def validate(self, context):
        ...
        This method is intended as a indicative measure of whether the NCC
    ...
    def deploy(self, context):
        ...
        NCC plugin. Every scheduled Node Driver will be assigned a Node of the
    ...
    def destroy(self, context):
        ...
        created by the NCC plugin as a consequence of the plumbing_info
        method belongs to the NCC plugin, and it is in charge of disposing
        them if needed.
