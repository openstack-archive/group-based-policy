...
class PolicyRuleSetRemoteSGsMapping(model_base.BASEV2):
    """PolicyRuleSet to SGs mapping DB."""

    __tablename__ = 'gpm_policy_rule_set_remote_sg_mapping'
...
def set_policy_rule_set_sg_mapping(session, policy_rule_set_id, consumed_sg_id,
    ...
    with session.begin(subtransactions=True):
        mapping = PolicyRuleSetRemoteSGsMapping(
...
def get_policy_rule_set_sg_mapping(session, policy_rule_set_id=None,
                                   ...
                                   l3_policy_ids=None):
    with session.begin(subtransactions=True):
        query = session.query(PolicyRuleSetRemoteSGsMapping)
        ...
        if l3_policy_ids is not None:
            query = query.filter(
                PolicyRuleSetRemoteSGsMapping.l3_policy_id.in_(l3_policy_ids))
        if tenant_id:
...
def increase_mapping_count(session, policy_rule_set_id, l3_policy_id,
                           tenant_id):
    with session.begin(subtransactions=True):
...
def decrease_mapping_count(session, policy_rule_set_id, l3_policy_id,
                           tenant_id):
    with session.begin(subtransactions=True):
        mapping = get_policy_rule_set_sg_mapping(session, policy_rule_set_id,
                                                 l3_policy_id, tenant_id)
...
class RemoteGroupManager(base.SecurityGroupManagerBase):
        ...
        if curr['l3_policies'] and (curr['external_routes'] !=
            ...
            for ep in eps:
                ...
                if cons:
                    ...
                    # PRS role
                    for prs in self._gbp_plugin.get_policy_rule_sets(
                            context._plugin_context, {'id': cons}):
                        # Add/Remove this ES rules
                        rules = self._gbp_plugin.get_policy_rules(
                            context._plugin_context,
                            {'id': prs['policy_rules']})
    ...
    def _delete_sg_pair(self, context, prs_id, l3p_id, tenant_id):
        ...
        with lockutils.lock('%s-%s-%s' % (prs_id, l3p_id, tenant_id),
                            'gbp-', True):
            plugin_context = self._admin_context
            decrease_mapping_count(plugin_context.session, prs_id, l3p_id,
                                   tenant_id)
            mapping = get_policy_rule_set_sg_mapping(
    ...
    def _add_or_remove_policy_rule_set_rule(
        ...
        l3p = self._gbp_plugin.get_l3_policy(
            self._admin_context, policy_rule_set_sg_mappings['l3_policy_id'])
    ...
    def _sg_ingress_rule(self, prs, sg_id, protocol, port_range,
        ...
        return self._sg_rule(
            ...
            unset=unset, ethertype=ethertype)
