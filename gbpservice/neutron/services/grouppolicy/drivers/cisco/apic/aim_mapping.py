...
class AIMMappingDriver(nrd.CommonNeutronBase, aim_rpc.AIMMappingRPCMixin):
    ...
    def _aim_application_profile_for_ptg(self, context, ptg, tenant_name=None):
        ...
        if apg_id:
            ...
            return self._aim_application_profile(
                context._plugin_context.session, apg, tenant_name=tenant_name)

    def _aim_application_profile(self, session, apg, tenant_name=None):
        # This returns a new AIM ApplicationProfile resource
        tenant_id = apg['tenant_id']
        if not tenant_name:
            tenant_name = self._aim_tenant_name(
                session, tenant_id,
                aim_resource_class=aim_resource.ApplicationProfile,
                gbp_obj=apg)
    ...
    def _create_aim_ap_for_ptg_conditionally(self, context, ptg,
        ...
        if ptg and ptg['application_policy_group_id'] and not (
            self._get_aim_application_profile_for_ptg(
                context, ptg, tenant_name=tenant_name)):
            ap = self._aim_application_profile_for_ptg(context, ptg,
                                                       tenant_name=tenant_name)
    ...
    def create_aim_ap_for_epg_conditionally(self, session, epg, new_tenant):
        ptg = session.query(gpmdb.PolicyTargetGroupMapping).filter_by(
            id=epg.name).first()
        if ptg:
            self._create_aim_ap_for_ptg_conditionally(session, ptg,
    ...
    def _delete_aim_ap_for_ptg_conditionally(self, context, ptg,
        ...
        if ptg and ptg['application_policy_group_id']:
            ...
            epgs = self.aim.find(aim_ctx, aim_resource.EndpointGroup,
                                 tenant_name=tenant_name,
            ...
            if not apg_db['policy_target_groups'] or (
                len(apg_db['policy_target_groups']) == 1 and (
                    apg_db['policy_target_groups'][0]['id'] == ptg['id']) or (
                    len(epgs) == 0)):
    ...
    def delete_aim_ap_for_epg_conditionally(self, session, epg, old_tenant):
        admin_context = self._get_admin_context_reuse_session(session)
        ptg = session.query(gpmdb.PolicyTargetGroupMapping).filter_by(
            id=epg.name).first()
        if ptg:
            self._delete_aim_ap_for_ptg_conditionally(admin_context, ptg,
                                                      tenant_name=old_tenant)
    ...
    def apic_ap_name_for_application_policy_group(self, session, apg_id,
                                                  name=None):
        return apg_id or self.aim_mech_driver.ap_name
