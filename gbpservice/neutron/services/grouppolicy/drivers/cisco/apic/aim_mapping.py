...
class AIMMappingDriver(nrd.CommonNeutronBase, aim_rpc.AIMMappingRPCMixin):
    ...
    def delete_policy_target_group_precommit(self, context):
        ...
        try:
            ...
            if self._is_redirect_in_policy_rule_sets(context,
                    context.current['provided_policy_rule_sets']):
                msg = "PTG Deletion failed, as chain deletion is going on in "
                "backend. Try again in sometime."
    ...
    def _is_redirect_in_policy_rule_sets(self, context, prs):
        policy_rule_ids = []
        for prs in context._plugin.get_policy_rule_sets(
                context._plugin_context, filters={'id':
                    prs}):
        ...
        for rule in context._plugin.get_policy_rules(
            ...
            redirect_actions = context._plugin.get_policy_actions(
                context._plugin_context, filters={'id': rule["policy_actions"],
                    'action_type': [g_const.gp_action_redirect]})
            if redirect_actions:
                return True
        return False
