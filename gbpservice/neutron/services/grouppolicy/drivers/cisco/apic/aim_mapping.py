...
class AIMMappingDriver(nrd.CommonNeutronBase, aim_rpc.AIMMappingRPCMixin):
    ...
    def _get_subnet_details(self, plugin_context, port, details):
        ...
        cached_port = hasattr(plugin_context, 'port_db')
        if hasattr(plugin_context, 'l2p_db'):
            l2p = plugin_context.l2p_db
        else:
        ...
        # REVISIT: See if we can use the cached subnet query.
        #          It still needs to be converted to a dict so
        #          that it can be used in the RPC
        subnets = self._get_subnets(
            plugin_context,
        ...
        for subnet in subnets:
            ...
            subnet_dhcp_ips = set()
            if cached_port:
                all_dhcp_ports = plugin_context.dhcp_db
            else:
                all_dhcp_ports = self._get_ports(
                    plugin_context,
                    filters={
                        'network_id': [subnet['network_id']],
                        'device_owner': [n_constants.DEVICE_OWNER_DHCP]})
            for dhcp_port in all_dhcp_ports:
    ...
    def _get_aap_details(self, plugin_context, port, details):
        ...
        # REVISIT: Should we turn this into a cached item
        #          or baked query?
    ...
    def _get_port_vrf(self, plugin_context, port):
        ...
        return self.aim_mech_driver.get_vrf_for_network(
            plugin_context.session, net_db, mapping=mapping)
    ...
    def _get_vrf_subnets(self, plugin_context, vrf_tenant_name, vrf_name,
        ...
        with session.begin(subtransactions=True):
            ...
            if hasattr(plugin_context, 'vrf_mapping'):
                mappings = plugin_context.vrf_mappings
            else:
                mappings = None
            address_scope_ids = (
            ...
            if address_scope_ids:
                for address_scope_id in address_scope_ids:
                    if hasattr(plugin_context, 'subnetpools_db'):
                        subnetpools_db = plugin_context.subnetpools_db
                        subnetpools = [{'prefixes': [prefix['cidr']]}
                                       for pool in subnetpools_db
                ...
                if hasattr(plugin_context, 'vrf_bds_db'):
                    bds = plugin_context.vrf_bds_db
                elif vrf_tenant_name != md.COMMON_TENANT_NAME:
                ...
                if net_ids:
                    # REVISIT: We should optimize this DB query
    ...
    def _get_nat_details(self, plugin_context, port, host, details):
        ...
        if hasattr(plugin_context, 'external_nets'):
            ext_nets = plugin_context.external_nets
        else:
            port_sn = set([x['subnet_id'] for x in port['fixed_ips']])
            router_intf_ports = self._get_ports(plugin_context,
                filters={
        ...
        for ext_net in ext_nets:
            if hasattr(ext_net, 'aim_extension_mapping'):
                dn = ext_net.aim_extension_mapping.external_network_dn
                nat_type = ext_net.aim_extension_mapping.nat_type
            else:
                dn = ext_net.get(cisco_apic.DIST_NAMES, {}).get(
                    cisco_apic.EXTERNAL_NETWORK)
            if hasattr(ext_net, 'aim_mapping'):
                epg = self.aim_mech_driver._get_network_epg(
                    ext_net.aim_mapping)
                ext_net_epg_dn = epg.dn
            else:
