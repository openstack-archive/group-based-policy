...
class AIMMappingDriver(nrd.CommonNeutronBase, aim_rpc.AIMMappingRPCMixin):
    ...
    def _check_for_default_subnetpool(self, context, ip_version):
        # First check in the current project
        pool = self.ml2plus_plugin.get_default_subnetpool(
            context._plugin_context.elevated(), ip_version=ip_version)
        return pool
    ...
    def create_l3_policy_precommit(self, context):
        ...
        for family in ip_dict.keys():
            ...
            default_pool = self._check_for_default_subnetpool(context,
                ip_version=family)
            if explicit_scope:
            ...
            if not explicit_pools and not default_pool:
    ...
    def _check_subnetpools_for_same_scope(self, context, subnetpools,
        ...
        for sp_id in subnetpools:
            ...
            if prefixes:
                prefixes = ', '.join(prefixes.split(',') + sp_prefixlist)
            else:
