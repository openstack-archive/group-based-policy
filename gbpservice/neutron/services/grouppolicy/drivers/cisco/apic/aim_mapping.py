...
class AIMMappingDriver(nrd.CommonNeutronBase, aim_rpc.AIMMappingRPCMixin):
    ...
    def _create_per_l3p_implicit_contracts(self):
        admin_context = n_context.get_admin_context()
        context = type('', (object,), {})()
        context._plugin_context = adim_context
        session = adim_context.s.ession
        aim_ctx = aim_context.AimContext(session)
        contract_name_prefix = alib.get_service_contract_filter_entries()[0]
        ...
        for l3p in l3ps:
            ...
            if not self.aim.find(
                ...
                self._create_implicit_contracts(admin_context, l3p)
