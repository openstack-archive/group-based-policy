...
class AIMMappingRPCMixin(ha_ip_db.HAIPOwnerDbMixin):
    ...
    # TODO(kentwu): Do some refactoring
    def _get_gbp_details_new(self, context, request, host):
        with context.session.begin(subtransactions=True):
            ...
            port_query = ("SELECT project_id, id, name, network_id, "
                          "mac_address, device_id, device_owner, "
                          "port_security_enabled, host, vif_type, "
                          "vif_details FROM "
            ...
            port = dict(port)
            """
            binding_level_query = ("SELECT * FROM "
            ...
            for binding_level in binding_levels:
                port['binding_levels'].append(
                    {'port_id': binding_level['port_id'],
                     'host': binding_level['host'],
                     'level': binding_level['level'],
                     'driver': binding_level['driver'],
