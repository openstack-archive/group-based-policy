...
class AIMMappingRPCMixin(ha_ip_db.HAIPOwnerDbMixin):
    ...
    """
    def _get_objects_for_cache(self, context, port_id, cache):
        with context.session.begin(subtransactions=True):
            # Get the port resource, and all its related resources
            port_db = (context.session.query(models_v2.Port).
                       outerjoin(ha_ip_db.HAIPAddressToPortAssocation,
                            models_v2.Port.id ==
                            ha_ip_db.HAIPAddressToPortAssocation.port_id).
                       filter(models_v2.Port.id == port_id))
            print str(port_db.statement.compile(dialect=postgresql.dialect()))
            port_db = port_db.one()
            cache.port_db = port_db

            # Get the network resource, and all it's related resources
            network_id = port_db.network_id
            network_db = (context.session.query(models_v2.Network).
                          filter(models_v2.Network.id == network_id).one())
            cache.network_db = network_db

            # Get the NetworkMapping resource
            network_mapping_db = (context.session.query(aim_db.NetworkMapping).
                filter(aim_db.NetworkMapping.network_id == network_id).
                one_or_none())
            cache.network_mapping_db = network_mapping_db
    """

    # TODO(kentwu): Continue to use prepared statements to make performance
    # improvement on get_bound_port_context() which has lots of calculations
    ...
    def _get_gbp_details_new(self, context, request, host):
        with context.session.begin(subtransactions=True):
            ...
            import pdb
            pdb.set_trace()
            self._get_objects_for_cache(context, port_id, context)
            """
            port_context = core_plugin.get_bound_port_context(context, port_id,
                                                              host)
            if not port_context:
            ...
            subnet_in_str = str(tuple(subnet_ids))
            # Remove the ',' at the end otherwise MySQL will complain
            if subnet_in_str[-1] == ')' and subnet_in_str[-2] == ',':
                subnet_in_str = subnet_in_str[0:-2] + subnet_in_str[-1]
            subnet_query = ("SELECT * FROM subnets WHERE "
                            "id in " + subnet_in_str)
            subnet_result = context.session.execute(subnet_query)
            ...
            for subnet in subnet_result:
                ...
                dns_query = ("SELECT address FROM dnsnameservers WHERE "
                             "subnet_id = '" + subnet['id'] + "'")
                dns_result = context.session.execute(dns_query)
                ...
                route_query = ("SELECT destination, nexthop FROM "
                               "subnetroutes WHERE "
                               "subnet_id = '" + subnet['id'] + "'")
                route_result = context.session.execute(route_query)
            ...
            dhcp_result = context.session.execute(dhcp_query)
            # Build up the ORM relationship manually
            dhcp_ports = []
            ...
            address_scope_query = (
                ...
                "vrf_name = '" + net_record['vrf_name'] + "'" + " AND "
                "vrf_tenant_name = '" + net_record['vrf_tenant_name'] + "'")
            as_result = context.session.execute(address_scope_query)
            subnetpools = []
            if as_result.rowcount > 0 or as_result.rowcount == -1:
                subnetpools_query = (
                    ...
                    "vrf_tenant_name = '" + net_record['vrf_tenant_name'] +
                    "'")
                subnetpools_res = context.session.execute(subnetpools_query)
                # Build up the ORM relationship manually
                for subnetpool in subnetpools_res:
                    ...
                    prefix_result = context.session.execute(prefix_query)
            ...
            vrf_subnets = []
            if as_result.rowcount == 0 or as_result.rowcount == -1:
                if net_record['vrf_tenant_name'] == md.COMMON_TENANT_NAME:
                    all_vrfs_bds_query = (
                        "SELECT name, tenant_name FROM aim_bridge_domains "
                        "WHERE vrf_name = '" + net_record['vrf_name'] + "'")
                    all_vrfs_bds_result = context.session.execute(
                                                        all_vrfs_bds_query)
                    all_vrfs_query = (
                        "SELECT tenant_name FROM aim_vrfs WHERE "
                        "name = '" + net_record['vrf_name'] + "'")
                    all_vrfs_result = context.session.execute(all_vrfs_query)
                    bd_tenants = set(
                    ...
                    aim_bd_query = (
                        ...
                        "WHERE vrf_name = '" + net_record['vrf_name'] + "'" +
                        " AND tenant_name = '" +
                        net_record['vrf_tenant_name'] + "'")
                    aim_bd_result = context.session.execute(aim_bd_query)
                net_ids = self._get_net_ids_from_bds(context.session,
                                                     aim_bd_result)
                if net_ids:
                    net_id_list = [str(r) for r in net_ids]
                    in_str = str(tuple(net_id_list))
                    # Remove the ',' at the end otherwise MySQL will complain
                    if in_str[-1] == ')' and in_str[-2] == ',':
                        in_str = in_str[0:-2] + in_str[-1]
                    vrf_subnet_query = ("SELECT cidr FROM subnets WHERE "
                                        "network_id in " + in_str)
                    vrf_subnet_result = context.session.execute(
                                                        vrf_subnet_query)
            ...
            routers = [str(p.device_id) for p in ports_result]
            in_str = str(tuple(routers))
            # Remove the ',' at the end otherwise MySQL will complain
            if in_str[-1] == ')' and in_str[-2] == ',':
                in_str = in_str[0:-2] + in_str[-1]
            ext_nets = []
            if routers:
                ...
                ext_nets = context.session.execute(ext_net_query)
            ...
            nested_allowed_vlans_query = (
                ...
                "network_id = '" + net_id + "'")
            nested_allowed_vlans_result = context.session.execute(
                                                    nested_allowed_vlans_query)
            net = dict(net_record)
            net['apic:nested_domain_allowed_vlans'] = []
            for allowed_vlan in nested_allowed_vlans_result:
                net['apic:nested_domain_allowed_vlans'].append(
                                                        allowed_vlan.vlan)
