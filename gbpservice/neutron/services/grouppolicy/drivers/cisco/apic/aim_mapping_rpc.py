...
class AIMMappingRPCMixin(ha_ip_db.HAIPOwnerDbMixin):
    ...
    def _get_objects_for_cache(self, context, port_id):
        with context.session.begin(subtransactions=True):
            ...
            # Get DHCP ports
            ports1 = orm.aliased(models_v2.Port)
            ports2 = orm.aliased(models_v2.Port)
            dhcp_query = self.bakery(lambda session:
                session.query(ports1))
            dhcp_query += lambda q: q.filter(
                ports1.device_owner.startswith('network:dhcp'))
            dhcp_query += lambda q: q.join(models_v2.IPAllocation)
            dhcp_query += lambda q: q.join(models_v2.Subnet)
            dhcp_query += lambda q: q.outerjoin(ports2)
            dhcp_query += lambda q: q.filter(
                ports2.network_id == ports1.network_id)
            dhcp_db = dhcp_query(context.session).all()
            ...
            #    for those BDs, and finally the CIDRs frmo the subnes on
            ...
            subnetpools_db = context.session.query(
                ...
                vrf_tenant_name=vrf.tenant_name, vrf_name=vrf.name)
            context.subnetpools_db = subnetpools_db

            vrf_bds_db = context.session.query(aim_models.BridgeDomain).filter(
                aim_models.BridgeDomain.tenant_name == vrf.tenant_name).filter(
                aim_models.BridgeDomain.vrf_name == vrf.name)
            context.vrf_bds_db = vrf_bds_db

            # REVISIT: get router ports for SNAT
