...
class PortForHAIPAddress(object):
    ...
    def _get_ha_ipaddress(self, port_id, ipaddress, writer_session=None):
        session = writer_session or db_api.get_reader_session()
    ...
    def get_ha_port_associations(self):
        session = db_api.get_reader_session()
        with session.begin(subtransactions=True):
            ports = session.query(HAIPAddressToPortAssocation).all()
        return ports
