...
class ApicMappingDriver(api.ResourceMappingDriver,
            ...
            while ptg['proxied_group_id']:
                ...
                for port in self._get_ptg_ports(proxied):
                    details['extra_ips'].extend([x['ip_address'] for x in
                                                 port['fixed_ips']])
                    fips, ipms, host_snat_ips = self._get_ip_mapping_details(
                        context, port['id'], l3_policy, host=details['host'])
                    details['floating_ip'].extend(fips)
                    details['ip_mapping'].extend(ipms)
                    details['host_snat_ips'].extend(host_snat_ips)
                ptg = proxied
        return details
    ...
    def _allocate_snat_ip_for_host_and_ext_net(self, context, host, network):
        ...
        if not snat_subnets:
            ...
            snat_ports = self.get_ports(context,
            ...
            if not snat_ports:
                ...
                attrs = {'device_id': DEVICE_ID_SNAT_PORT,
                ...
                port = self.create_port(context, attrs)
    ...
    def create_external_segment_precommit(self, context):
        ...
        if ext_info and ext_info.get('cidr_exposed'):
            ...
            if self._is_nat_enabled_on_es(es) and 'host_pool_cidr' in ext_info:
    ...
    def _plug_external_policy_to_segment(self, context, ep, segments,
        ...
        if segments:
            ...
            for es in added_ess:
                ...
                nat_enabled = self._is_nat_enabled_on_es(es)
                if nat_enabled and not is_shadow:
                    ep_name = self.name_mapper.external_segment(context, es,
                        prefix="default-")
                with self.apic_manager.apic.transaction() as trs:
        ...
        if segments:
            ...
            for es in added_ess:
                ...
                last_ep = not [x for x in es['external_policies']
                               if x != ep['id']]
                if not self._is_nat_enabled_on_es(es) or is_shadow or last_ep:
    ...
    def _check_l3p_es(self, context):
        ...
        if l3p['external_segments']:
            ...
            for es in ess:
                if (not self._is_nat_enabled_on_es(es) and
                        [x for x in es['l3_policies'] if x != l3p['id']]):
            ...
            if gw and plen:
                ...
                networks = self._get_networks(
                        context._plugin_context, filters={
                            ...
                            'tenant_id': [es['tenant_id']]})
                if not networks:
                    LOG.warning(_("External network %(name)s not found for "
