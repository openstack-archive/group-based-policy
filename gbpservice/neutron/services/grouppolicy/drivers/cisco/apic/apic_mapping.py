...
class PTGChangeDisallowedWithNonACINetwork(gpexc.GroupPolicyBadRequest):
...
class ApicMappingDriver(api.ResourceMappingDriver,
    ...
    def create_policy_target_precommit(self, context):
        ...
        # TODO(amitbose) checks
        # PT port should have unique address in L2P network
    ...
    def update_policy_target_precommit(self, context):
        ...
        if (context.original['policy_target_group_id'] !=
                context.current['policy_target_group_id']):
            if self._is_supported_non_apic_port(context,
                                                context.current['port_id']):
                raise PTGChangeDisallowedWithNonACINetwork(
    ...
    def _is_supported_non_apic_network_type(self, net_type):
        return (net_type != ofcst.TYPE_OPFLEX and
                net_type in [p_const.TYPE_VLAN])

    def _is_supported_non_apic_network(self, network):
        return self._is_supported_non_apic_network_type(
            network[providernet.NETWORK_TYPE])

    def _is_supported_non_apic_port(self, context, port_id):
        port = self._get_port(context._plugin_context, port_id)
        network = self._get_network(context._plugin_context,
                                    port['network_id'])
        return self._is_supported_non_apic_network(network)

    def _ptg_needs_shadow_network(self, context, ptg):
        net = self._l2p_id_to_network(context._plugin_context,
                                      ptg['l2_policy_id'])
        return self._is_supported_non_apic_network(net)
    ...
    def _sync_shadow_subnets(self, context, l2p, old, new, ptg=None):
        network = self._get_network(context, l2p['network_id'])
        if not self._is_supported_non_apic_network(network):
            return
        ...
        for grp in ptgs:
            ...
            try:
                if old and not new:
                    if shadow_subnet:
                        self._delete_subnet(context, shadow_subnet[0]['id'])
                elif new and not old:
                    attrs.update({
                        'tenant_id': shadow_net['tenant_id'],
                        'name': '%ssub_%s' % (APIC_OWNED, ref_subnet['id']),
                        'network_id': shadow_net['id']})
                    self._create_subnet(context, attrs)
                else:
                    if shadow_subnet:
                        self._update_subnet(context, shadow_subnet[0]['id'],
                                            attrs)
            except Exception:
                LOG.exception(
    ...
    def _get_static_binding_info_for_port(self, context, use_original):
        bound_seg = (context.original_bottom_bound_segment if use_original else
                     context.bottom_bound_segment)
        if not bound_seg or not self._is_supported_non_apic_network_type(
