...
from gbpservice.neutron.services.grouppolicy.drivers import resource_mapping as api
...
class ApicMappingDriver(api.ResourceMappingDriver):
    ...
    def initialize(self):
        ...
        self._gbpservice_plugin = None
        ApicMappingDriver.me = self

    @property
    def gbpservice_plugin(self):
        if not self._gbpservice_plugin:
            self._gbpservice_plugin = (manager.NeutronManager.get_service_plugins()
                                .get("GROUP_POLICY"))
        return self._gbpservice_plugin

    @staticmethod
    def get_initialized_instance():
        return ApicMappingDriver.me

    def get_gbpservice_details(self, context, **kwargs):
        ...
        if not kwargs.get('policy_target'):
            ...
            ptg = self.gbpservice_plugin.get_policy_target_group(
            ...
            if ptg:
                ...
                attrs = {'policy_target':
                         {'tenant_id': port['tenant_id'],
                          ...
                          'port_id': port['id']}}
                self.gbpservice_plugin.create_policy_target(plugin_context, attrs)
    ...
    def create_policy_rule_set_postcommit(self, context):
        ...
        with self.apic_manager.apic.transaction(None) as trs:
            ...
            rules = self.gbpservice_plugin.get_policy_rules(
                context._plugin_context,
    ...
    def process_path_deletion(self, context, port, policy_target=None):
        port_details = self.get_gbpservice_details(
    ...
    def _manage_policy_target_port(self, plugin_context, pt):
        ...
        if port.get('binding:host_id'):
            ...
            if port_details:
                ...
                plugin_context._plugin = self.gbpservice_plugin
                plugin_context._plugin_context = plugin_context
                ptg_object = self.gbpservice_plugin.get_policy_target_group(
    ...
    def _manage_ptg_policy_rule_sets(
        ...
        with self.apic_manager.apic.transaction(transaction) as trs:
            for x in xrange(len(provided)):
                for c in self.gbpservice_plugin.get_policy_rule_sets(
    ...
    def _manage_ptg_subnets(self, plugin_context, ptg, added_subnets,
        ...
        plugin_context._plugin = self.gbpservice_plugin
        plugin_context._plugin_context = plugin_context
        l2_policy_object = self.gbpservice_plugin.get_l2_policy(
    ...
    def _delete_path_if_last(self, context, port_info):
        if not self._get_active_path_count(context, port_info):
            # TODO(ivar): change APICAPI to not expect a resource context
            context._plugin = self.gbpservice_plugin
    ...
    def _get_default_security_group(self, context, ptg_id, tenant_id):
        ...
        filters = {'name': ['gbpservice_apic_default'], 'tenant_id': [tenant_id]}
        default_group = self._core_plugin.get_security_groups(
            context, filters)
        if not default_group:
            attrs = {'name': 'gbpservice_apic_default', 'tenant_id': tenant_id,
    ...
    def _port_id_to_ptg(self, context, port_id):
        ...
        if pt:
            return self.gbpservice_plugin.get_policy_target_group(
                context, pt['policy_target_group_id'])
        return

    def _l2p_id_to_network(self, context, l2p_id):
        l2_policy = self.gbpservice_plugin.get_l2_policy(context, l2p_id)
        return self._core_plugin.get_network(context, l2_policy['network_id'])

    def _network_id_to_l2p(self, context, network_id):
        l2ps = self.gbpservice_plugin.get_l2_policies(
