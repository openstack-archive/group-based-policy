...
class DefaultPTGNameConflict(gpexc.GroupPolicyBadRequest):
    message = _("Default PTG with name " + DEFAULT_PTG + " exists."
                "User creation of default PTG is not supported.")


class DefaultPTGUpdateNotSupported(gpexc.GroupPolicyBadRequest):
    message = _("Update of default PTG with name " + DEFAULT_PTG + " not "
                "supported.")
...
class ApicMappingDriver(api.ResourceMappingDriver,
        ...
        if self.create_default_ptg:
            default_ptg = self._db_plugin(
                context._plugin).get_policy_target_group(
                    context._plugin_context, context.current['id'])
            if default_ptg['name'] == DEFAULT_PTG:
                raise DefaultPTGUpdateNotSupported()
    ...
    def update_l2_policy_precommit(self, context):
        ...
        # TODO(Sumit): Update shared attribute of default PTG
