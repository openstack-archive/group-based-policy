...
class NsxPolicyMappingDriver(api.ResourceMappingDriver):
    ...
    def _create_domain(self, project_id):
    ...
    def create_policy_target_group_postcommit(self, context):
        ...
        if len(project_ptgs) == 1:
            ...
            self._create_domain(project_id)
    ...
    def delete_policy_rule_set_postcommit(self, context):
        ...
        for comm_profile in (in_name(ruleset_id), out_name(ruleset_id)):
            try:
                self.nsx_policy.comm_profile.delete(comm_profile)
            except nsxlib_exc.ResourceNotFound:
                # certain direction can not exist on backend
                pass
