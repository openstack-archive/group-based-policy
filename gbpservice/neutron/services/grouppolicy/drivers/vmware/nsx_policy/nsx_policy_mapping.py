...
class NsxPolicyMappingDriver(api.ResourceMappingDriver):
    ...
    def _create_communication_profile(self, profile_id, description, rules):
        ...
        # REVISIT(annak) - create with parent is preferrable
        #self.policy_api.create_with_parent(profile, entry)
        self.policy_api.create(profile)
        self.policy_api.create(entry)
    ...
    def _map_provided_rule_set(self, project_id, ptgs, group_id, ruleset_id):
        ...
        if ruleset_in:
            self.cmap_seq += 1
            cm = policy.CommunicationMapEntryDef(
                    project_id,
                    map_id=ruleset_in,
    ...
    def _set_sg_rules_for_subnets(self, context, subnets,
        ...
        self.cmap_seq = 0
        communication_maps = self._map_group_rule_sets(
                context, group_id,
                provided_policy_rule_sets,
                consumed_policy_rule_sets)

        if not communication_maps:
            return

        parent = policy.CommunicationMapDef(project_id)
        self.policy_api.create_with_parent(parent,
                                           communication_maps)
    ...
    def _unset_sg_rules_for_subnets(self, context, subnets,
        ...
        group = policy.GroupDef(project_id, group_id)
