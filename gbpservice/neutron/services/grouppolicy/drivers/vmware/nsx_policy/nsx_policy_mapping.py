...
class NsxPolicyMappingDriver(api.ResourceMappingDriver):
    ...
    def _create_domain(self, context):
        ...
        domain_name = self._generate_nsx_name(
            project_id, context._plugin_context.tenant_name)

        LOG.info(_LI('Creating domain %(domain)s for project %(project)s'),
                {'domain': domain_name,
                 'project': project_id})

        self.nsx_policy.domain.create(name=domain_name,
                                      domain_id=project_id)

        self.nsx_policy.deployment_map.create(name=domain_name,
                                              map_id=project_id,
                                              domain_id=project_id,
                                              ep_id=SINGLE_ENTRY_ID)
    ...
    def _delete_comm_profile(self, comm_profile_id):
        ...
        try:
            ...
            # certain direction may not exist on backend
            pass
    ...
    def delete_policy_rule_set_postcommit(self, context):
        ...
        for comm_profile in (in_name(ruleset_id), out_name(ruleset_id)):
            self._delete_comm_profile(comm_profile)
