...
class ResourceMappingDriver(api.PolicyDriver, local_api.LocalAPI,
    ...
    def create_l2_policy_precommit(self, context):
        ...
        if not context.current['inject_dhcp_routes']:
            raise exc.UnsettingInjectDHCPRoutesOfL2PolicyNotSupported()
    ...
    def update_l2_policy_precommit(self, context):
        if (context.current['inject_dhcp_routes'] !=
            context.original['inject_dhcp_routes']):
            raise exc.UnsettingInjectDHCPRoutesOfL2PolicyNotSupported()
        if (context.current['l3_policy_id'] !=
