...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def delete_network_service_policy_precommit(self, context):
        context.nsp_cleanup_ips = {}
        for ptg_id in context.current.get("policy_target_groups"):
            ipaddress = self._get_ptg_policy_ipaddress_mapping(
                    context._plugin_context.session, ptg_id)
            if ipaddress:
                context.nsp_cleanup_ips[ptg_id] = ipaddress
    ...
    def delete_network_service_policy_postcommit(self, context):
        for ptg_id in context.current.get("policy_target_groups"):
            ...
            self._cleanup_network_service_policy(
                context, subnet, ptg_id, context.nsp_cleanup_ips.get(ptg_id))
    ...
    def _validate_nsp_parameters(self, context):
        ...
        if (nsp_params and (
                len(nsp_params) > 1 or nsp_params[0].get("type") != "ip_single"
