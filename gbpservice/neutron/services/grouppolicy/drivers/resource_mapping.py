...
from gbpservice.network.neutronv2 import api as q_api
...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def _reject_non_shared_net_on_shared_l2p(self, context):
        if context.current.get('shared') and context.current['network_id']:
            net = self._get_network(context._plugin_context, context.current['network_id'])
    ...
    def create_external_segment_precommit(self, context):
        if context.current['subnet_id']:
            subnet = self._get_subnet(context._plugin_context, context.current['subnet_id'])
            network = self._get_network(context._plugin_context, subnet['network_id'])
    ...
    def _update_port(self, plugin_context, port_id, attrs):
        return self._update_neutron_resource(plugin_context, 'port', port_id, attrs)
    ...
    def _update_subnet(self, plugin_context, subnet_id, attrs):
        return self._update_neutron_resource(plugin_context, 'subnet', subnet_id, attrs)
    ...
    def _get_network(self, plugin_context, network_id):
        return self._get_neutron_resource(plugin_context, 'network', network_id)
    ...
    def _update_router(self, plugin_context, router_id, attrs):
        return self._update_neutron_resource(plugin_context, 'router', router_id, attrs)
    ...
    def _create_security_group(self, plugin_context, attrs):
        return self._create_neutron_resource(plugin_context, 'security_group', attrs)

    def _get_security_group(self, plugin_context, sg_id):
        return self._get_neutron_resource(plugin_context, 'security_group', sg_id)

    def _get_security_groups(self, plugin_context, filters={}):
        return self._get_neutron_resources(plugin_context, 'security_group', filters)

    def _update_security_group(self, plugin_context, sg_id, attrs):
        return self._update_neutron_resource(plugin_context, 'security_group', sg_id, attrs)

    def _delete_security_group(self, plugin_context, sg_id):
        self._delete_neutron_resource(plugin_context, 'security_group', sg_id)

    def _create_security_group_rule(self, plugin_context, attrs):
        try:
            return self._create_neutron_resource(plugin_context, 'security_group_rule', attrs)
    ...
    def _get_security_group_rule(self, plugin_context, sg_rule_id):
        return self._get_neutron_resource(plugin_context, 'security_group_rule', sg_rule_id)

    def _get_security_group_rules(self, plugin_context, filters={}):
        return self._get_neutron_resources(plugin_context, 'security_group_rule', filters)
    ...
    def _disassoc_sgs_from_port(self, plugin_context, port_id, sg_list):
        try:
            ...
            updated_port= dict([(ext_sg.SECURITYGROUPS, new_sg_list)])
            self._update_port(plugin_context, port_id, updated_port)
        # TODO(yi): handle PortNotFound with REST API
        except n_exc.PortNotFound:
            LOG.warn(_("Port %s is missing") % port_id)
