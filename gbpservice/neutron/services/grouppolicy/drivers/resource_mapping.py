...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def create_policy_target_postcommit(self, context):
        ...
        if context.current.get('proxy_gateway'):
            self._set_proxy_gateway_routes(context)
    ...
    def delete_policy_target_postcommit(self, context):
        ...
        for fip in context.fips:
            self._delete_fip(context._plugin_context,
                             fip.floatingip_id)
        if context.current.get('proxy_gateway'):
            self._unset_proxy_gateway_routes(context)
    ...
    def create_policy_target_group_postcommit(self, context):
        ...
        if subnets:
            l3p = self._get_ptg_l3p(context, context.current)
    ...
    def _stitch_ptg_to_l3p(self, context, l3p, subnet_ids):
        try:
    ...
    def _validate_proxy_ptg(self, context):
        ...
        if current.get('proxied_group_id'):
    ...
    def _update_proxy_gateway_routes(self, context, unset=False):
        pt = context.current
        ptg = context._plugin.get_policy_target_group(
        ...
        for fixed_ip in port['fixed_ips']:
            if fixed_ip.get('ip_address'):
                ...
                break
        routes = []
        # TODO(ivar): also need to set proper routes in the proxy PTG subnets
        # if any
        if nexthop:
            subnets = self._core_plugin.get_subnets(
                context._plugin_context, {'id': proxied['subnets']})
            for subnet in subnets:
                routes.append((subnet['cidr'], nexthop))
            if unset:
                self._update_l3p_routes(l3p, remove=set(routes))
            else:
                self._update_l3p_routes(l3p, add=set(routes))

    def _set_proxy_gateway_routes(self, context):
        self._update_proxy_gateway_routes(context)

    def _unset_proxy_gateway_routes(self, context):
        self._update_proxy_gateway_routes(context, unset=True)
