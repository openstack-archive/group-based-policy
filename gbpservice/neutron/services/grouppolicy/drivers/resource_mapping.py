...
class ImplicitResourceOperations(local_api.LocalAPI,
    ...
    def _allocate_proxy_prefix(self, context, l3p):
        pool = netaddr.IPSet(gbp_utils.convert_ip_pool_string_to_list(
            l3p['proxy_ip_pool']))
        prefixlen = l3p['proxy_subnet_prefix_length']
        allocated = netaddr.IPSet(
            iterable=self._get_l3p_allocated_subnets(context, l3p['id']))

        available = pool - allocated
        available.compact()

        cidrs = sorted(available.iter_cidrs(),
                       key=operator.attrgetter('prefixlen'),
                       reverse=True)
        if not cidrs or prefixlen < cidrs[0].prefixlen:
            raise n_exc.NoSubnetAvailable()

        return cidrs[0].subnet(prefixlen).next()

    def _add_prefix_to_subnetpool(self, context, subnetpool_id, prefix):
        subnetpool = self._get_subnetpool(context._plugin_context,
                                          subnetpool_id)
        if prefix in subnetpool['prefixes']:
            return
        update = {'prefixes': subnetpool['prefixes'] + [prefix]}
        self._update_subnetpool(context._plugin_context,
                                subnetpool_id,
                                update)

    def _remove_prefix_from_subnetpool(self, context, subnetpool_id, prefix):
        subnetpool = self._get_subnetpool(context._plugin_context,
                                          subnetpool_id)
        update = {'prefixes':
                  list(set(subnetpool['prefixes']) - set([prefix]))}
        self._update_subnetpool(context._plugin_context,
                                subnetpool_id,
                                update)
...
class ResourceMappingDriver(api.PolicyDriver, ImplicitResourceOperations,
               ...
               (is_proxy and
                ...
                try:
                    ...
                    if context.current.get('proxied_group_id'):
                        # This is a temporary solution for proxy prefix
                        # allocation. When subnetpools are used, neutron
                        # forces subnets on same network to be allocated
                        # from same subnetpool. For this purpose, we add
                        # proxy prefix to the subnetpool just before the
                        # subnet is to be allocated.
                        proxy_cidr = self._allocate_proxy_prefix(context, l3p)
                        ip_version = netaddr.IPNetwork(proxy_cidr).version
                        self._add_prefix_to_subnetpool(
                            context,
                            l3p[self.L3P_SUBNETPOOLS_KEYS[ip_version]][0],
                            proxy_cidr)
                        subnet_specifics = {'cidr': proxy_cidr}
    ...
    def delete_policy_target_group_precommit(self, context):
        ...
        if (MAPPING_CFG.use_subnetpools and
            context.current.get('proxied_group_id')):
            # This is proxy group - we need to cleanup prefixes from pool
            l2p = context._plugin.get_l2_policy(
                context._plugin_context, context.current['l2_policy_id'])
            l3p = context._plugin.get_l3_policy(
                context._plugin_context, l2p['l3_policy_id'])
            ...
            for subnet_id in subnets:
                ...
                self._remove_prefix_from_subnetpool(
                    context,
                    l3p[self.L3P_SUBNETPOOLS_KEYS[ip_version]][0],
                    subnet['cidr'])
