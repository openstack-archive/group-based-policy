...
class ResourceMappingDriver(api.PolicyDriver):
        ...
        try:
            ...
            if proxied.get('proxied_group_id'):
                ...
                gateway_pt = context._plugin.get_policy_targets(
                    ...
                    {'policy_target_group_id': [proxied['proxied_group_id']],
                     'proxy_gateway': [True]})
                if gateway_pt:
    ...
    def _update_ptg_routes(self, ptg, add=None, remove=None):
        ...
        remove = remove or set()
        admin_context = n_context.get_admin_context()
        subnets = self._core_plugin.get_routers(
            admin_context, {'id': ptg['subnets']})
    ...
    def _update_proxy_gateway_routes(self, context, pt, unset=False):
        ...
        if nexthop:
            ...
            # If the PTG is itself proxied, the route will be
            # set in the proxy PTG subnets, in the L3P routers otherwise
            if ptg['proxy_group_id']:
                func = self._update_ptg_routes
                param = context._plugin.get_policy_target_group(
                    context._plugin_context, ptg['proxy_group_id'])
            else:
                func = self._update_l3p_routes
                param = l3p

            if unset:
                func(param, remove=set(routes))
            else:
                func(param, add=set(routes))
