...
class ResourceMappingDriver(api.PolicyDriver, local_api.LocalAPI):
    ...
    def update_nat_pool_precommit(self, context):
        ...
        if (context.original['external_segment_id'] !=
            ...
            self._set_db_np_subnet(context, None)
            self._add_nat_pool_to_segment(context)
    ...
    def _set_db_np_subnet(self, context, subnet_id):
        context.current['subnet_id'] = subnet_id
        db_np = context._plugin._get_nat_pool(
            context._plugin_context, context.current['id'])
        db_np.subnet_id = context.current['subnet_id']
    ...
    def _add_nat_pool_to_segment(self, context):
        ...
        if not external_segment['subnet_id']:
            # REVISIT(ivar): do we really need this?
        ...
        if match:
            ...
            self._set_db_np_subnet(context, match[0]['id'])
    ...
    def _use_implicit_nat_pool_subnet(self, context):
        ...
        self._set_db_np_subnet(context, subnet['id'])
