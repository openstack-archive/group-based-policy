...
class ResourceMappingDriver(api.PolicyDriver, ImplicitResourceOperations,
    ...
    def delete_policy_target_group_postcommit(self, context):
        LOG.info(_LI("delete_policy_target_group_postcommit: %(ptg)s"), {
            'ptg': context.current})
        try:
            if not context.current['proxied_group_id']:
                self._cleanup_network_service_policy(context,
                                             context.current,
                                             context.nsp_cleanup_ipaddress,
                                             context.nsp_cleanup_fips)
        except Exception:
            LOG.info(_LI("cleanup nsp fails for %(ptg)s, %(nsp_ip)s, "
                "%(nsp_fips)s"), {'ptg': context.current,
                    'nsp_ip': context.nsp_cleanup_ipaddress,
                    'nsp_fips': context.nsp_cleanup_fips})
        # Cleanup SGs
        self._unset_sg_rules_for_subnets(
