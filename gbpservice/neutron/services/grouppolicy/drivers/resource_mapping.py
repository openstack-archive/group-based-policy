...
class ResourceMappingDriver(api.PolicyDriver):
    ...
    def _allocate_floating_ip(self, context, l2_policy_id, fixed_port=None):
        ...
        fip_ids = []
        ext_net_id = None
        # Allocate one floating IP per External Segment.Floating IP is
        # allocated from the Nat Pool, In Juno, Neutron does not allow choosing
        # a particular IP or allocation pool for Floating IP. So for now we are
        # forcing the nat pool to compose the same ip range as the external
        # subnet. In kilo we have to change the nat_pool to map to a subnet
        # allocation pool on the external subnet rather than a cidr
