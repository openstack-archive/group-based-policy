...
from gbpservice.neutron.services.grouppolicy.drivers import neutron_api_mixin as mixin
    ...
    def _reject_invalid_network_access(self, context):
        ...
        if context.current['network_id']:
            ...
            try:
                ...
                # REVISIT(yi): keep neutron exception handling for UT
                raise exc.InvalidNetworkAccess(
                    msg="Can't access other tenants networks",
                    network_id=context.current['network_id'],
                    tenant_id=context.current['tenant_id'])
    ...
    def _sg_rule(self, plugin_context, tenant_id, sg_id, direction,
        ...
        if unset:
            ...
            try:
                ...
                # REVISIT(yi): keep neutron exception handling for UT
                # patched with plugin calls
                LOG.warn(_('Security Group already exists: %s'), ex.message)
                return
            except webob.exc.HTTPClientError as ex:
                # catch the exception in UT when patched with WSGI
                if ex.code==409 and 'security_group_rule' in ex.detail:
                    LOG.warn(_('Security Group already exists: %s'))
                    return
                else:
                    raise
