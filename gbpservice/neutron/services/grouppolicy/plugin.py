...
class GroupPolicyPlugin(group_policy_mapping_db.GroupPolicyMappingDbPlugin):
    ...
    def _create_resource_db(self, context, resource_type, resource):
        ...
        with session.begin(subtransactions=True):
            ...
            create_ob_precommit(policy_context)
    ...
    def create_policy_target_bulk(self, context, policy_targets):
        objects = self.create_bulk_gbp(
            gp_cts.POLICY_TARGET, context, policy_targets)
    ...
    def create_bulk_gbp(self, context, resource_type, request_items):
        ...
        with db_api.context_manager.writer.using(context):
            for item in items:
                try:
                    ...
                    result, policy_context = self._create_resource_db(
                        context, resource_type, item)
                    objects.append({'policy_context': policy_context,
                                    'result': result,
                                    'attributes': attrs})
        ...
        for obj in objects:
            try:
                postcommit_op(context, obj['result'], obj['policy_context'])
            except Exception:
                with excutils.save_and_reraise_exception():
                    resource_ids = [res['result']['id'] for res in objects]
                    LOG.exception("GBP _after_create_%(res)s "
                                  "failed for %(res)s: "
