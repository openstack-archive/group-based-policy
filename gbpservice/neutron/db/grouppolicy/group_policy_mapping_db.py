...
class GroupPolicyMappingDbPlugin(gpdb.GroupPolicyDbPlugin):
    ...
    def validate_ip_pool(ip_pool, ip_version):
        ...
        if type(ip_pool) is unicode or type(ip_pool) is str:
            ip_pool = gpdb.convert_string_to_ip_pool(ip_pool)
    ...
    def validate_subnet_prefix_length(ip_version, new_prefix_length,
        ...
        if not ip_pool:
            ip_pool = []
        if ((new_prefix_length > gpdb.MAX_IPV4_SUBNET_PREFIX_LENGTH) or
                (new_prefix_length < 2)):
    ...
    def create_l3_policy(self, context, l3_policy):
        ...
        with context.session.begin(subtransactions=True):
            l3p_db = L3PolicyMapping(id=uuidutils.generate_uuid(),
                                     ...
                                     ip_pool=gpdb.convert_ip_pool_to_string(
                                         ip_pool),
        ...
        return self._make_l3_policy_dict(l3p_db, ip_pool=ip_pool)
        ...
        with context.session.begin(subtransactions=True):
            ...
            if 'subnet_prefix_length' in l3p:
                self.validate_subnet_prefix_length(l3p_db.ip_version,
                    l3p['subnet_prefix_length'],
                    gpdb.convert_string_to_ip_pool(l3p_db.ip_pool))
