...
class LastResortSubnetpoolMixin(object):
    ...
    def get_subnetpool_of_last_resort_id(self, context, tenant=None):
        pool = self.get_subnetpool_of_last_resort(context, tenant=tenant)
    ...
    def update_last_resort_subnetpool(self, context, subnetpool):
        ...
        with context.session.begin(subtransactions=True):
            if is_last_resort:
                ...
                # tenant (or global)
                msg = _('There can be at most one last resort '
                        'subnetpool per tenant.')
                self._validate_last_resort_subnetpool(
                    context, subnetpool['id'], tenant=subnetpool['tenant_id'],
                    msg=msg)
                if subnetpool['shared']:
                    # Check globally too
                    msg = _('There can be at most one global last resort '
                            'subnetpool.')
                    self._validate_last_resort_subnetpool(
    ...
    def _validate_last_resort_subnetpool(self, context, subnetpool_id,
        ...
        if (len(current_last_resort_sp) == 1 and
                current_last_resort_sp[0]['id'] != subnetpool_id):
            raise n_exc.BadRequest(resource='subnetpools', msg=msg)
