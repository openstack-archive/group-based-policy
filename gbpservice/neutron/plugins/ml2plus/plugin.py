...
class Ml2PlusPlugin(ml2_plugin.Ml2Plugin):
    ...
    def create_network(self, context, network):
        tenant_id = network[attributes.NETWORK]['tenant_id']
    ...
    def create_network_bulk(self, context, networks):
        for network in networks[attributes.NETWORKS]:
            tenant_id = network[attributes.NETWORK]['tenant_id']
            self.mechanism_manager.ensure_tenant(context, tenant_id)
        return super(Ml2PlusPlugin, self).create_network_bulk(context,
                                                              networks)

    # TODO(rkukura): Override remaining create operations to ensure
    # tenant.

    # TODO(rkukura): Override address_scope, subnet_pool, and other
    # resources to invoke mechanism and extension drivers.
