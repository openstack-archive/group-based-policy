...
def _add_network_mapping(session, network_id, bd, epg, vrf, ext_net=None):
    if not ext_net:
        ...
        session.execute(NetworkMapping.insert().values(
            network_id=network_id, l3out_name=ext_net.l3out_name,
            l3out_ext_net_name=ext_net.name,
            l3out_tenant_name=ext_net.tenant_name, vrf_name=vrf.name,
            vrf_tenant_name=vrf.tenant_name))
...
def _get_network_extn_db(session, network_id):
    _, ext_net_dn, nat_type = (session.query(NetworkExtensionDb).filter_by(
