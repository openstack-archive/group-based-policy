...
class ApicExtensionDriver(api_plus.ExtensionDriver,
    ...
    def process_update_network(self, plugin_context, data, result):
        ...
        # Fact that a network is SVI cannot be updated currently.
        # if this changes handle bgp updates with current status
        # of whether network is SVI
        if not result.get(cisco_apic.SVI):
            return
        bgp_enable_trigger = False
        if cisco_apic.BGP in data:
            if not result.get(cisco_apic.BGP) and data.get(cisco_apic.BGP):
            ...
            result.update(res_dict)
        # BGP disable trigger
        if not result.get(cisco_apic.BGP):
            if result.get(cisco_apic.BGP_TYPE) != "":
                res_dict = {cisco_apic.BGP_TYPE: "",
                            cisco_apic.ASN: "0"}
                result.update(res_dict)
            return
        if cisco_apic.BGP_TYPE in data:
            ...
            self.set_network_extn_db(plugin_context.session, result['id'],
                                     res_dict)
            result.update(res_dict)
        elif bgp_enable_trigger:
            res_dict = {cisco_apic.BGP_TYPE: "default_export",
                        cisco_apic.ASN: "0"}
            self.set_network_extn_db(plugin_context.session, result['id'],
                                     res_dict)
            result.update(res_dict)
