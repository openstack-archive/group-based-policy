...
class NetworkExtensionDb(model_base.BASEV2):
    ...
    network_id = sa.Column(
        ...
        primary_key=True)
    external_network_dn = sa.Column(sa.String(1024), nullable=False)
    nat_type = sa.Column(sa.Enum('distributed', 'edge', ''), nullable=False)
...
class SubnetExtensionDb(model_base.BASEV2):
    ...
    subnet_id = sa.Column(
        ...
        primary_key=True)
    snat_host_pool = sa.Column(sa.Boolean, nullable=False)
...
class ExtensionDbMixin(object):
    ...
    def get_network_extn_db(self, session, network_id):
        ...
        result = {}
        if db_obj:
            result[cisco_apic.EXTERNAL_NETWORK] = db_obj['external_network_dn']
            result[cisco_apic.NAT_TYPE] = db_obj['nat_type']
    ...
    def set_network_extn_db(self, session, network_id, res_dict):
        with session.begin(subtransactions=True):
            ...
            if not db_obj['external_network_dn']:
                return False
            session.add(db_obj)
    ...
    def get_subnet_extn_db(self, session, subnet_id):
        ...
        return ({cisco_apic.SNAT_HOST_POOL: db_obj['snat_host_pool']}
                if db_obj else {})
