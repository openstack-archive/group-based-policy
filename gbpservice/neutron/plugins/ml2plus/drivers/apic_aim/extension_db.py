...
class ExtensionDbMixin(object):
    ...
    def get_network_extn_db_bulk(self, session, network_ids):
        query = self.bakery(lambda s: s.query(NetworkExtensionDb))
        query += lambda q: q.filter(
            NetworkExtensionDb.network_id.in_(
                sa.bindparam('network_ids', expanding=True)))
        db_objs = query(session).params(network_ids=network_ids).all()

        query = self.bakery(lambda s: s.query(NetworkExtensionCidrDb))
        query += lambda q: q.filter(
            NetworkExtensionCidrDb.network_id.in_(
                sa.bindparam('network_ids', expanding=True)))
        db_cidrs = query(session).params(network_ids=network_ids).all()

        query = self.bakery(
            lambda s: s.query(NetworkExtNestedDomainAllowedVlansDb))
        query += lambda q: q.filter(
            NetworkExtNestedDomainAllowedVlansDb.network_id.in_(
                sa.bindparam('network_ids', expanding=True)))
        db_vlans = query(session).params(network_ids=network_ids).all()
    ...
    def set_network_extn_db(self, session, network_id, res_dict):
        with session.begin(subtransactions=True):
            ...
            db_obj = query(session).params(network_id=network_id).first()
    ...
    def get_network_ids_by_ext_net_dn(self, session, dn, lock_update=False):
        query = self.bakery(lambda s: s.query(NetworkExtensionDb.network_id))
        query += lambda q: q.filter_by(external_network_dn=sa.bindparam('dn'))
        if lock_update:
    ...
    def get_external_cidrs_by_ext_net_dn(self, session, dn, lock_update=False):
        ...
        query = self.bakery(lambda s: s.query(ctab.cidr))
        query += lambda q: q.join(ntab, ntab.network_id == ctab.network_id)
        query += lambda q: q.filter(
    ...
    def get_subnet_extn_db(self, session, subnet_id):
        query = self.bakery(lambda s: s.query(SubnetExtensionDb))
        query += lambda q: q.filter_by(subnet_id=sa.bindparam('subnet_id'))
        db_obj = query(session).params(subnet_id=subnet_id).first()
    ...
    def get_router_extn_db(self, session, router_id):
        query = self.bakery(lambda s: s.query(RouterExtensionContractDb))
        query += lambda q: q.filter_by(router_id=sa.bindparam('router_id'))
        db_contracts = query(session).params(router_id=router_id).all()
