...
class DbModel(object):
    ...
    def __init__(self):
        ...
        # REVISIT(rkukura): Do we need to instead pass in the session
        # for each call so these updates are part of the caller's
        # transaction? This will require changes to the NameMapper
        # class.
        self.session = db_api.get_session()

    def add_apic_name(self, neutron_id, neutron_type, apic_name):
        LOG.info(_LI("APIC AIM DbModel add_apic_name id: %(id)s, "
    ...
    def update_apic_name(self, neutron_id, neutron_type, apic_name):
        with self.session.begin(subtransactions=True):
            name = self.session.query(old_model.ApicName).filter_by(
    ...
    def get_apic_names(self):
        return self.session.query(old_model.ApicName).all()

    def get_apic_name(self, neutron_id, neutron_type):
        return self.session.query(old_model.ApicName.apic_name).filter_by(
            neutron_id=neutron_id, neutron_type=neutron_type).first()

    def delete_apic_name(self, neutron_id):
        with self.session.begin(subtransactions=True):
            try:
                self.session.query(old_model.ApicName).filter_by(
    ...
    def get_filtered_apic_names(self, neutron_id=None, neutron_type=None,
                                apic_name=None):
        query = self.session.query(old_model.ApicName.apic_name)
        if neutron_id:
