...
class DbModel(object):
    ...
    def update_apic_name(self, session, neutron_id, neutron_type, apic_name):
        with session.begin(subtransactions=True):
            name = session.query(old_model.ApicName).filter_by(
            ...
            if name:
                ...
                self.add_apic_name(session, neutron_id, neutron_type,
                                   apic_name)

    def get_apic_names(self, session):
        return session.query(old_model.ApicName).all()
