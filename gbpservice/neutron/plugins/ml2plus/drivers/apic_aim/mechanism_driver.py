...
class ApicMechanismDriver(api_plus.MechanismDriver):
    ...
    def create_subnet_precommit(self, context):
        ...
        network = self.plugin.get_network(context._plugin_context,
                                          network_id)
        if self._is_external(network) and subnet['gateway_ip']:
            l3out, ext_net, ns = self._get_aim_nat_strategy(network)
            if not ext_net:
                return  # Unmanaged external network
            session = context._plugin_context.session
    ...
    def update_subnet_precommit(self, context):
        ...
        network = self.plugin.get_network(context._plugin_context,
                                          network_id)
        is_ext = self._is_external(network)
    ...
    def add_router_interface(self, context, router, port, subnets):
        ...
        # Number of existing router interface ports exlcuding the
