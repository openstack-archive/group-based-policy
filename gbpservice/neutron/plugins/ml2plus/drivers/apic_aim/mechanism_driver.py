...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def validate_aim_mapping(self, mgr):
        # REVISIT: This is a huge function that needs to be refactored
        # into a set of smaller functions once its overall flow of
        # control and information is fully understood.
        ...
        for klass in [aim_infra.HostDomainMappingV2,
                      aim_resource.PhysicalDomain,
                      aim_resource.VMMDomain]:
            for resource in mgr.aim_mgr.find(mgr.actual_aim_ctx, klass):
                mgr.aim_mgr.create(mgr.expected_aim_ctx, resource)

        # Register DB tables to be generically validated.
        ...
        for scope_db in mgr.actual_session.query(as_db.AddressScope).all():
        ...
        for router_db in mgr.actual_session.query(l3_db.Router).all():
            ...
            # is called for the project that owns the router, so that
            # project's resources must be expected. Remove this once
            # per-project resources are managed more dynamically.
            tenant_name = self.name_mapper.project(
                mgr.expected_session, router_db.tenant_id)
            self._expect_tenant(mgr, tenant_name)

        # Get external contracts for routers.
        router_external_provided_contracts = defaultdict(set)
        router_external_consumed_contracts = defaultdict(set)
        for contract in mgr.actual_session.query(
                extension_db.RouterExtensionContractDb).all():
            if contract.provides:
                router_external_provided_contracts[contract.router_id].add(
                    contract.contract_name)
            else:
                router_external_consumed_contracts[contract.router_id].add(
                    contract.contract_name)

        # Find all router interfaces for each routed network.
        intfs = (mgr.actual_session.query(l3_db.RouterPort,
        ...
        for intf in intfs:
            _, _, _, network, _ = intf
            routed_nets[network.id].append(intf)
        ...
        for intfs in routed_nets.values():
            ...
            router_ids = set()
            for intf in intfs:
                router_port, _, subnet, network, scope_mapping = intf
                router_ids.add(router_port.router_id)
                if not net_id:
                    net_id = network.id
                if scope_mapping:
                    if subnet.ip_version == 4:
                        if (v4_scope_mapping and
                            v4_scope_mapping != scope_mapping):
                            # REVISIT: Fail validation.
                            print("Inconsistent IPv4 scopes for network %s" %
                                  intfs)
                        else:
                            v4_scope_mapping = scope_mapping
                    elif subnet.ip_version == 6:
                        if (v6_scope_mapping and
                            v6_scope_mapping != scope_mapping):
                            # REVISIT: Fail validation.
                            print("Inconsistent IPv6 scopes for network %s" %
                                  intfs)
                        else:
                            v6_scope_mapping = scope_mapping
            ...
            if scope_mapping:
                ...
                unscoped_net_dbs[net_id] = network
                if self._network_shared(network):
                    shared_unscoped_net_ids.append(network.id)
        ...
        # Get actual AIM EPGs, since we currently cannot compute the
        # exected value for certain fields. See REVISIT comments where
        # these are used below for details.
        #
        # REVISIT: This find is redundant with the ValidationManager
        # getting all actual resources to compare. Change
        # ValidationManager to find actual instances when resource
        # class is registered, and make these available to drivers.
        actual_epgs = {tuple(actual_epg.identity): actual_epg
                       for actual_epg in mgr.aim_mgr.find(
                               mgr.actual_aim_ctx, aim_resource.EndpointGroup)}

        # Validate networks.
        for net_db in mgr.actual_session.query(models_v2.Network).all():
            bd = None
            epg = None
            vrf = None
            ext_net = None
            if net_db.external:
                ...
                if ext_net:
                    # REVISIT: Avoid peicemeal queries against the
                    # actual DB throughout this code.

                    # REVISIT: We probably need to copy the external
                    # network's pre-existing resources, if they are
                    # monitored, from the actual AIM store to the
                    # validation AIM store, so that the NatStrategy
                    # behaves as expected during validation. But the
                    # current external connectivity UTs don't actually
                    # create pre-existing ExternalNetwork resources.
                    ...
                    ns.update_external_cidrs(
                        mgr.expected_aim_ctx, ext_net, cidrs)
                    ...
                    # REVISIT: Process each AIM ExternalNetwork rather
                    # than each external Neutron network?
                    ...
                    for key, routers in vrf_routers.items():
                        ...
                        for router_id in routers:
                            contract = self._map_router(
                                mgr.expected_session, router_dbs[router_id],
                                True)
                            prov.add(contract.name)
                            cons.add(contract.name)
                            prov.update(
                                router_external_provided_contracts[router_id])
                            cons.update(
                                router_external_consumed_contracts[router_id])
                        ext_net.provided_contract_names = sorted(prov)
                        ext_net.consumed_contract_names = sorted(cons)
                        int_vrf = int_vrfs[key]
                        # Keep only the identity attributes of the VRF
                        # so that calls to nat-library have consistent
                        # resource values. This is mainly required to
                        # ease unit-test verification. Note that this
                        # also effects validation of the L3Outside's
                        # display_name.
                        int_vrf = aim_resource.VRF(
                            tenant_name=int_vrf.tenant_name, name=int_vrf.name)
                        ns.connect_vrf(
                            mgr.expected_aim_ctx, ext_net, int_vrf)
                # Only SVI networks persist ext_net.
                ext_net = None
            elif self._is_svi_db(net_db):
                print("validation of SVI networks is not yet implemented")
            else:
                ...
                for intf in routed_nets.get(net_db.id, []):
                    router_port, ip_allocation, subnet_db, _, _ = intf
            ...
            # values.
            if epg:
                actual_epg = actual_epgs.get(tuple(epg.identity))
                if actual_epg:
            ...
            # REVISIT: Although networks map to resources that may be
            # under a different Tenant, ensure_tenant is called for
            # the project that owns the network, so that project's
            # resources must be expected. Remove this once per-project
            # resources are managed more dynamically.
            tenant_name = self.name_mapper.project(
                mgr.expected_session, net_db.tenant_id)
            self._expect_tenant(mgr, tenant_name)
    ...
    def _expect_tenant(self, mgr, tenant_name):
        ...
        if not mgr.expected_aim_resource(tenant):
            ...
            project_id = self.name_mapper.reverse_project(
                mgr.expected_session, tenant_name)
            project_name = self.project_name_cache.get_project_name(project_id)
