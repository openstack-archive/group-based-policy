...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _associate_domain(self, port_context, is_vmm=True):
        ...
        try:
            if is_vmm:
                ...
                if aim_hd_mappings:
                    domains = [{'name': aim_hd_mapping.domain_name}
                               for aim_hd_mapping in aim_hd_mappings]
                if not domains:
                    vmms, phys = self.get_aim_domains(aim_ctx)
                    self.aim.update(aim_ctx, epg,
    ...
    def disassociate_domain(self, port_context, use_original=False):
        ...
        if (self._is_opflex_type(btm[api.NETWORK_TYPE]) or
            ...
            def _extend_mappings(aim_hd_mappings,
                                 domain_types, domains, hd_mappings):
                for mapping in aim_hd_mappings:
                    domain_type = mapping.domain_type
                    if domain_type in domain_types and mapping.domain_name:
            ...
            if self._is_opflex_type(btm[api.NETWORK_TYPE]):
                _extend_mappings(aim_hd_mappings, ['OpenStack', 'VMware'],
                                 domains, hd_mappings)
            else:
                _extend_mappings(aim_hd_mappings, ['PhysDom'],
                                 domains, hd_mappings)
            if not domains:
                return
            hosts = [x.host_name
