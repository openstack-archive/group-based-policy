    ...
    def _update_static_path_for_svi(self, session, port_context, network,
        ...
        if new_path:
            ...
            for node_path in node_paths:
                ...
                aim_l3out_node = aim_resource.L3OutNode(
                    ...
                    node_path=node_path, router_id='9.9.9.9',
        ...
        if is_vpc:
            ...
            filters = {'network_id': [network['id']],
                       'name': ['apic-svi-port:node-%s' % switch]}
            svi_ports = self.plugin.get_ports(port_context._plugin_context,
                                              filters)
            if svi_ports:
                primary_ip = svi_ports[0]['fixed_ips'][0]['ip_address']
            else:
                attrs = {'device_id': '',
                         ...
                         'mac_address': n_constants.ATTR_NOT_SPECIFIED,
                         'fixed_ips': [{'subnet_id': network['subnets'][0]}],
                         'admin_state_up': False}
                port = self.plugin.create_port(port_context._plugin_context,
                                               {'port': attrs})
                if port and port['fixed_ips']:
                    primary_ip = port['fixed_ips'][0]['ip_address']
                else:
