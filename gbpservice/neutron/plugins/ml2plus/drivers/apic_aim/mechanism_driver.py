...
class ApicMechanismDriver(api_plus.MechanismDriver):
    ...
    def delete_address_scope_precommit(self, context):
        ...
        # At this point all persistent DB state about the address-scope
        # has been deleted. So we cannot use _map_address_scope() to
        # locate the mapped VRF. Fortunately, 'current' is constructed
        # prior to deletion of DB state, and can be used to locate the
        # mapped VRF.
        vrf_dn = current.get(cisco_apic.DIST_NAMES, {}).get(cisco_apic.VRF)
        if vrf_dn:
            vrf = self.aim.get(aim_ctx, aim_resource.VRF.from_dn(vrf_dn))
            if vrf:
                if not vrf.monitored:
                    self.aim.delete(aim_ctx, vrf)
                else:
                    self.aim.update(aim_ctx, vrf, display_name='')
