...
from gbpservice.neutron.services.grouppolicy.drivers.cisco.apic import (
    port_ha_ipaddress_binding as ha_ip)
    ...
    def _check_allowed_address_pairs(self, context, port):
        ...
        if aap_current != aap_original:
            ...
            for aap in removed:
                ...
                with p_context.session.begin(subtransactions=True) as session:
                    # Get all the owned IP addresses for the port, and if
                    # they match a removed AAP entry, delete that entry
                    # from the DB
                    ha_ips = session.session.query(
                        ha_ip.HAIPAddressToPortAssocation).filter_by(
                                    port_id=port['id']).all()
                    for ip in ha_ips:
                        if ip.ha_ip_address in cidr:
