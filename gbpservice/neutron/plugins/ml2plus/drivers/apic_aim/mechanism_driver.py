...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _request_endpoint_details(self, context, request, host):
        ...
        while True:
            ...
            with db_api.context_manager.reader.using(context) as session:
                ...
                # REVISIT: Remove this and all following commented-out prints.
                ...
                if port_info.vif_type not in [
                    ...
                    info['sg_info'] = self._query_endpoint_sg_info(
                        session, port_info.port_id)
                    # print("got sg_info: %s" % info['sg_info'])
                    ...
                    info['aap_info'] = self._query_endpoint_aap_info(
                        session, port_info.port_id)
                    # print("got aap_info: %s" % info['aap_info'])
                    ...
                    # print("got owned_ip_info: %s" % info['owned_ip_info'])
                    ...
                    info['ext_net_info'] = self._query_endpoint_ext_net_info(
                        session, subnet_ids)
                    # print("got ext_net_info: %s" % info['ext_net_info'])
                    ...
                    # print("got fip_info: %s" % info['fip_info'])
                    ...
                    # print("got snat_info: %s" % info['snat_info'])
    ...
    def _query_endpoint_aap_info(self, session, port_id):
        query = BAKERY(lambda s: s.query(
            n_addr_pair_db.AllowedAddressPair.mac_address,
    ...
    def _query_endpoint_ext_net_info(self, session, subnet_ids):
        ...
        query += lambda q: q.join(
            l3_db.RouterPort,
            l3_db.RouterPort.router_id == l3_db.Router.id and
            l3_db.RouterPort.port_type == n_constants.DEVICE_OWNER_ROUTER_INTF)
        query += lambda q: q.join(
    ...
    def _query_endpoint_fip_info(self, session, port_ids):
        ...
        query = BAKERY(lambda s: s.query(
            ...
            l3_db.FloatingIP.fixed_ip_address,
        ))
        query += lambda q: q.filter(
            l3_db.FloatingIP.fixed_port_id.in_(sa.bindparam(
