...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def validate_aim_mapping(self, mgr):
        ...
        for intfs in routed_nets.values():
            ...
            if v4_scope_mapping:
                network_vrfs[net_id] = self._get_address_scope_vrf(
                    v4_scope_mapping)
            elif v6_scope_mapping:
                # REVISIT: If there are also unscoped v4 subnets,
                # should the unscoped topology's default VRF be used
                # instead? Or should validation fail?
                network_vrfs[net_id] = self._get_address_scope_vrf(
                    v6_scope_mapping)
            else:
                unscoped_net_router_ids[net_id] = router_ids
                for router_id in router_ids:
                    unscoped_router_net_ids.setdefault(
                        router_id, set()).add(net_id)
