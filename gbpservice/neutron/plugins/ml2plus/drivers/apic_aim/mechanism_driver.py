...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _get_default_ext_epg(self, network_db):
        l3out = self._get_svi_net_l3out(network_db)
        default_ext_epg = aim_resource.ExternalNetwork(
            tenant_name=l3out.tenant_name, l3out_name=l3out.name,
            name=L3OUT_EXT_EPG)
        return default_ext_epg
    ...
    def add_router_interface(self, context, router, port, subnets):
        ...
        if not net_intfs:
            ...
            if network_db.aim_mapping.epg_name:
                ...
                epg = self._get_default_ext_epg(network_db)
    ...
    def _associate_network_with_vrf(self, ctx, aim_ctx, network_db, new_vrf,
        ...
        if not self._is_svi_db(network_db):
            ...
            default_ext_net = self._get_default_ext_epg(network_db)
    ...
    def _get_svi_net_l3out(self, network_db):
        if self._is_svi_db(network_db):
            l3out_vrf = self._get_network_vrf(network_db.aim_mapping)
            name = self._scope_default_svi_l3out_name_by_vrf(l3out_vrf.name)
            return aim_resource.L3Outside(
                tenant_name=l3out_vrf.tenant_name, name=name)
