...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _setup_default_arp_dhcp_security_group_rules(self):
        session = db_api.get_session()
        aim_ctx = aim_context.AimContext(session)

        sg = aim_resource.SecurityGroup(
            tenant_name=COMMON_TENANT_NAME, name='aim_default')
        # this default SG has already been created
        if self.aim.get(aim_ctx, sg):
            return
        self.aim.create(aim_ctx, sg)

        sg_subject = aim_resource.SecurityGroupSubject(
            tenant_name=COMMON_TENANT_NAME,
            security_group_name='aim_default', name='default')
        self.aim.create(aim_ctx, sg_subject)

        arp_egress_rule = aim_resource.SecurityGroupRule(
            tenant_name=COMMON_TENANT_NAME,
            security_group_name='aim_default',
        ...
        dhcp_ingress_rule = aim_resource.SecurityGroupRule(
            ...
            to_port='68',
            conn_track='normal')
        self.aim.create(aim_ctx, arp_egress_rule)
        self.aim.create(aim_ctx, arp_ingress_rule)
        self.aim.create(aim_ctx, dhcp_egress_rule)
        self.aim.create(aim_ctx, dhcp_ingress_rule)
