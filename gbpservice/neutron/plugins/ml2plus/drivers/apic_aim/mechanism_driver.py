...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _query_endpoint_port_info(self, session, port_id):
        ...
        return [EndpointPortInfo._make(port) for port in
                query(session).params(
                    port_id=port_id)]
    ...
    def _query_endpoint_static_ip_info(self, session, port_id):
        ...
        return [EndpointStaticIpInfo._make(ip) for ip in
                query(session).params(
                    port_id=port_id)]
    ...
    def _query_endpoint_binding_info(self, session, port_id):
        ...
        query += lambda q: q.order_by(
            models.PortBindingLevel.level)
        return [EndpointBindingInfo._make(level) for level in
                query(session).params(
                    port_id=port_id)]
    ...
    def _build_subnet_details(self, info):
        details = []
        for ip in info['ip_info']:
            subnet = {}
            subnet['cidr'] = ip.cidr
            details.append(subnet)
        return details
