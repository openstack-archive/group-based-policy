...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _insert_provisioning_block(self, context):
        ...
        port = context.current
        if not context.host or port['status'] == n_constants.PORT_STATUS_ACTIVE:
        ...
        vnic_type = port.get(portbindings.VNIC_TYPE,
                             portbindings.VNIC_NORMAL)
        if vnic_type not in SUPPORTED_VNIC_TYPES:
            LOG.debug("Refusing to insert provisioning_block due to unsupported vnic_type: %s",
                      vnic_type)
            return

        if context.host_agents(ofcst.AGENT_TYPE_OPFLEX_OVS) or context.host_agents(AGENT_TYPE_DVS):
