...
class ApicMechanismDriver(api_plus.MechanismDriver):
    ...
    def _associate_domain(self, port_context, is_vmm=True):
        ...
        if is_vmm:
            ...
            #domain = aim.look_up_physdom(host_id)
            domain = 'pysdom'
            domain = None
            if not domain:
                vmms, phys = self.get_aim_domains(aim_ctx)
    ...
    def _disassociate_domain(self, port_context, use_original=False):
        ...
        if (self._is_opflex_type(btm[api.NETWORK_TYPE]) or
            ...
            # TBD: query hosts under this vmm
            #domain = aim.look_up_vmm(host_id)
            #domain = 'ostack'
            domain = None
            if not domain:
                return
            hosts = ['f4-compute-1.noiro.lab', 'f4-compute-2.noiro.lab']
