...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def create_subnet_precommit(self, context):
        ...
        # Provide VRF notifications if creating unscoped
        scope_id = self._get_address_scope_id_for_subnets(
            context._plugin_context, [current])
        if not network_db.external and scope_id == NO_ADDR_SCOPE:
    ...
    def _notify_vrf_for_scope_id(self, context, scope_id):
        session = context._plugin_context.session
    ...
    def create_subnetpool_precommit(self, context):
        current = context.current
        scope_id = current.get('address_scope_id')
        if scope_id:
            self._notify_vrf_for_scope_id(context, scope_id)
    ...
    def update_subnetpool_precommit(self, context):
        ...
        if current_scope_id and current_prefixes != original_prefixes:
            self._notify_vrf_for_scope_id(context, current_scope_id)
    ...
    def _associate_network_with_vrf(self, ctx, aim_ctx, network_db, new_vrf,
        ...
        old_vrf = self._get_network_vrf(network_db.aim_mapping)
        old_scope_ids = self._get_address_scope_ids_for_vrf(ctx.session,
                                                            old_vrf)
        if scope_id == NO_ADDR_SCOPE:
            self._add_vrf_notification(new_vrf, vrfs_to_notify=vrfs_to_notify)
        if old_vrf != new_vrf and not old_scope_ids:
            self._add_vrf_notification(old_vrf, vrfs_to_notify=vrfs_to_notify)
    ...
    def _dissassociate_network_from_vrf(self, ctx, aim_ctx, network_db,
        ...
        if scope_id == NO_ADDR_SCOPE:
            self._add_vrf_notification(old_vrf, vrfs_to_notify=vrfs_to_notify)
        new_scope_ids = self._get_address_scope_ids_for_vrf(ctx.session,
                                                            new_vrf)
        if new_vrf != old_vrf and not new_scope_ids:
            self._add_vrf_notification(new_vrf, vrfs_to_notify=vrfs_to_notify)
    ...
    def _get_address_scope_id_for_subnets(self, context, subnets):
        ...
        subnetpool_id = subnets[0].get('subnetpool_id') if subnets else None
        if subnetpool_id:
