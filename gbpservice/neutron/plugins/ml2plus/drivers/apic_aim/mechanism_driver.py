...
class ApicMechanismDriver(api_plus.MechanismDriver):
    ...
    def get_gbp_details(self, context, **kwargs):
        # TODO(rkukura): Change to debug or info
        LOG.warning(_LW("APIC AIM MD handling get_gbp_details for: %s"),
                    kwargs)
        ...
        with session.begin(subtransactions=True):
            ...
            epg_tenant_name = str(
                self.name_mapper.tenant(session, network['tenant_id']))
            epg_name = str(
                self.name_mapper.network(session, network['id'], None))
        ...
        details = {'allowed_address_pairs': port['allowed_address_pairs'],
                   'app_profile_name': AP_NAME,
                   'device': device,
                   'enable_dhcp_optimization': True,  # REVISIT: config?
                   'enable_metadata_optimization': True,  # REVISIT: config?
