...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def __init__(self):
        ...
        self.bakery_size_exceeded = False
        self.bakery = baked.bakery(1000, self._bakery_size_alert)

    def _bakery_size_alert(self, cache):
        if not self.bakery_size_exceeded:
            LOG.warning("sqlalchemy baked query cache size exceeded")
            self.bakery_size_exceeded = True
    ...
    def create_subnet_precommit(self, context):
        ...
        if network_db.external is not None and current['gateway_ip']:
            ...
            if other_nets:
                query = self.bakery(lambda s: s.query(models_v2.Subnet.cidr))
        ...
        if self._is_svi_db(network_db):
            query = self.bakery(lambda s: s.query(models_v2.Subnet))
            query += lambda q: q.filter(
                models_v2.Subnet.network_id == sa.bindparam('network_id'))
            subnets_size = query(session).params(network_id=network_id).count()
    ...
    def extend_subnet_dict(self, session, subnet_db, result):
        ...
        query = self.bakery(lambda s: s.query(models_v2.Network))
        query += lambda q: q.filter_by(id=sa.bindparam('network_id'))
    ...
    def update_subnetpool_precommit(self, context):
        ...
        if current_scope_id != original_scope_id:
            ...
            query = self.bakery(lambda s: s.query(l3_db.RouterPort))
            ...
            rps = query(session).params(pool_id=pool_id).all()
    ...
    def update_router(self, context, current, original):
        ...
        if current['name'] != original['name']:
            ...
            # extend_router_dict.
            query = self.bakery(lambda s: s.query(models_v2.IPAllocation))
            query += lambda q: q.join(
                l3_db.RouterPort,
            ...
            query += lambda q: q.filter(
                ...
                l3_db.RouterPort.port_type ==
                n_constants.DEVICE_OWNER_ROUTER_INTF)
            for intf in query(session).params(router_id=current['id']):

                # TODO(rkukura): Avoid separate queries for these.
                query = self.bakery(lambda s: s.query(models_v2.Subnet))
                query += lambda q: q.filter_by(id=sa.bindparam('subnet_id'))
    ...
    def extend_router_dict(self, session, router_db, result):
        ...
        query = self.bakery(lambda s: s.query(
            models_v2.IPAllocation.ip_address, models_v2.Subnet,
            models_v2.Network))
        query += lambda q: q.join(
            models_v2.Subnet,
            models_v2.Subnet.id == models_v2.IPAllocation.subnet_id)
        ...
        for intf in query(session).params(router_id=router_db.id):
    ...
    def add_router_interface(self, context, router, port, subnets):
        ...
        # the DB session.
        query = self.bakery(lambda s: s.query(
            l3_db.RouterPort.router_id, models_v2.Subnet))
        query += lambda q: q.join(
            models_v2.IPAllocation,
        ...
        net_intfs = query(session).params(network_id=network_id).all()
    ...
    def remove_router_interface(self, context, router_id, port, subnets):
        ...
        query = self.bakery(lambda s: s.query(l3_db.Router))
        query += lambda q: q.filter_by(id=sa.bindparam('router_id'))
        router_db = query(session).params(router_id=router_id).one()
        ...
        query = self.bakery(lambda s: s.query(l3_db.RouterPort.router_id))
        query += lambda q: q.join(
            models_v2.Port,
            models_v2.Port.id == l3_db.RouterPort.port_id)
        query += lambda q: q.filter(
        ...
        router_ids = [r[0] for r in
                      query(session).params(network_id=network_id)]
    ...
    def _really_update_sg_rule_with_remote_group_set(
        ...
        query = self.bakery(lambda s: s.query(sg_models.SecurityGroupRule))
        query += lambda q: q.filter(
    ...
    def create_security_group_rule_precommit(self, context):
        ...
        if sg_rule.get('remote_group_id'):
            ...
            query = self.bakery(lambda s: s.query(models_v2.Port))
            query += lambda q: q.join(
    ...
    def _get_vrfs_for_router(self, session, router_id):
        ...
        query += lambda q: q.filter(
            l3_db.RouterPort.port_type == n_constants.DEVICE_OWNER_ROUTER_INTF)
        query += lambda q: q.distinct()
        scope_dbs = query(session).params(router_id=router_id)
        ...
        network_db = query(session).params(router_id=router_id).first()
    ...
    def _get_routers_for_vrf(self, session, vrf):
        ...
        if scope_ids:
            ...
            query += lambda q: q.distinct()
            rtr_dbs = query(session).params(scope_ids=scope_ids)
        else:
            ...
            query += lambda q: q.filter(
                ...
                n_constants.DEVICE_OWNER_ROUTER_INTF)
            query += lambda q: q.distinct()
            rtr_dbs = query(session).params(net_ids=net_ids)
        return rtr_dbs
    ...
    def _expand_topology_for_routers(self, session, visited_networks,
        ...
        if added_ids:
            ...
            query = self.bakery(lambda s: s.query(
                models_v2.Network, models_v2.Subnet))
            query += lambda q: q.join(
                models_v2.Subnet,
                models_v2.Subnet.network_id == models_v2.Network.id)
            query += lambda q: q.join(
    ...
    def _subnet_router_ips(self, session, subnet_id):
        query = self.bakery(lambda s: s.query(
            models_v2.IPAllocation.ip_address, l3_db.RouterPort.router_id))
        query += lambda q: q.join(
            l3_db.RouterPort,
            l3_db.RouterPort.port_id == models_v2.IPAllocation.port_id)
        query += lambda q: q.filter(
            ...
            l3_db.RouterPort.port_type == n_constants.DEVICE_OWNER_ROUTER_INTF)
        return query(session).params(subnet_id=subnet_id)
    ...
    def _get_router_intf_count(self, session, router, scope_id=None):
        if not scope_id:
            ...
            query += lambda q: q.filter(
                l3_db.RouterPort.port_type ==
                n_constants.DEVICE_OWNER_ROUTER_INTF)
            result = query(session).params(router_id=router['id']).count()
            ...
            result = query(session).params(
                router_id=router['id'], scope_ids=scope_ids).count()
    ...
    def _notify_port_update_for_fip(self, plugin_context, port_id):
        ...
        if fixed_ips:
            ...
            addr_pair = query(plugin_context.session).params(
                network_id=port['network_id'], fixed_ips=fixed_ips).all()
    ...
    def get_or_allocate_snat_ip(self, plugin_context, host_or_vrf,
        ...
        snat_port = query(session).params(
            network_id=ext_network['id'], device_id=host_or_vrf).first()
    ...
    def _delete_snat_ip_ports_if_reqd(self, plugin_context,
        ...
        if not query(session).params(
            ...
            query = self.bakery(lambda s: s.query(models_v2.Port.id))
            query += lambda q: q.filter(
                models_v2.Port.network_id == sa.bindparam('ext_network_id'),
                models_v2.Port.device_owner == DEVICE_OWNER_SNAT_PORT)
    ...
    def get_subnets_for_fip(self, context, floatingip):
        ...
        query = self.bakery(lambda s: s.query(models_v2.Subnet.id))
        query += lambda q: q.outerjoin(
            extn_db_sn,
            extn_db_sn.subnet_id == models_v2.Subnet.id)
        query += lambda q: q.filter(
    ...
    def _update_static_path(self, port_context, host=None, segment=None,
        ...
        if remove:
            ...
            query = self.bakery(lambda s: s.query(models.PortBindingLevel))
    ...
    def disassociate_domain(self, port_context, use_original=False):
        ...
        if (self._is_opflex_type(btm[api.NETWORK_TYPE]) or
            ...
            def _bound_port_query(session, port, hosts=None):
                ...
                query += lambda q: q.filter(
                    models.PortBindingLevel.port_id != sa.bindparam('port_id'))
                ports = query(session).params(hosts=hosts, port_id=port['id'])
                return ports
    ...
    def _get_non_opflex_segments_on_host(self, context, host):
        ...
        query = self.bakery(lambda s: s.query(segments_model.NetworkSegment))
        query += lambda q: q.join(
        ...
        segments = query(session).params(host=host).all()
    ...
    def _get_router_interface_subnets(self, session, router_id):
        ...
        query += lambda q: q.distinct()
        subnet_ids = query(session).params(router_id=router_id)
    ...
    def _get_non_router_ports_in_subnets(self, session, subnet_ids):
        ...
        query = self.bakery(lambda s: s.query(models_v2.IPAllocation.port_id))
        query += lambda q: q.join(
            models_v2.Port,
        ...
        query += lambda q: q.filter(
            ...
            n_constants.DEVICE_OWNER_ROUTER_INTF)
        port_ids = query(session).params(subnet_ids=subnet_ids).all()
    ...
    def _get_non_router_ports_in_networks(self, session, network_ids):
        ...
        query += lambda q: q.filter(
            models_v2.Port.device_owner !=
            n_constants.DEVICE_OWNER_ROUTER_INTF)
        port_ids = query(session).params(network_ids=list(network_ids)).all()
