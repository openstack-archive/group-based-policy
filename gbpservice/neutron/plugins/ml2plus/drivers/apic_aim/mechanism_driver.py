...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def validate_aim_mapping(self, mgr):
        ...
        self._validate_static_resources(mgr, data)
        self._validate_address_scopes(mgr, data)
        self._validate_routers(mgr, data)
        self._get_router_ext_contracts(mgr, data)
        self._determine_vrfs(mgr, data)
        self._validate_networks(mgr, data)
        self._validate_security_groups(mgr, data)
        self._validate_ports(mgr, data)
        self._validate_subnetpools(mgr, data)
        self._validate_floatingips(mgr, data)

    def _validate_static_resources(self, mgr, data):
    ...
    def _validate_address_scopes(self, mgr, data):
    ...
    def _validate_routers(self, mgr, data):
        router_dbs = {}
        ...
        data['router_dbs'] = router_dbs
        data['ext_net_routers'] = ext_net_routers

    def _get_router_ext_contracts(self, mgr, data):
        # Get external contracts for routers.
        router_ext_provided_contracts = defaultdict(set)
        router_ext_consumed_contracts = defaultdict(set)
        for contract in mgr.actual_session.query(
                extension_db.RouterExtensionContractDb):
            if contract.provides:
                router_ext_provided_contracts[contract.router_id].add(
                    contract.contract_name)
            else:
                router_ext_consumed_contracts[contract.router_id].add(
                    contract.contract_name)

        data['router_ext_provided_contracts'] = router_ext_provided_contracts
        data['router_ext_consumed_contracts'] = router_ext_consumed_contracts

    def _determine_vrfs(self, mgr, data):
        ...
        data['routed_nets'] = routed_nets
        data['network_vrfs'] = network_vrfs
        data['router_vrfs'] = router_vrfs

    def _validate_networks(self, mgr, data):
        for net_db in mgr.actual_session.query(models_v2.Network):
    ...
    def _validate_normal_network(self, mgr, data, net_db):
        network_vrfs = data['network_vrfs']
        router_dbs = data['router_dbs']
    ...
    def _validate_external_network(self, mgr, data, net_db):
        ext_net_routers = data['ext_net_routers']
        router_vrfs = data['router_vrfs']
        router_dbs = data['router_dbs']
        router_ext_provided_contracts = data['router_ext_provided_contracts']
        router_ext_consumed_contracts = data['router_ext_consumed_contracts']
        ...
        for key, routers in vrf_routers.items():
            ...
            for router_id in routers:
                ...
                prov.add(contract.name)
                cons.add(contract.name)
                prov.update(router_ext_provided_contracts[router_id])
                cons.update(router_ext_consumed_contracts[router_id])
            ext_net.provided_contract_names = sorted(prov)
            ext_net.consumed_contract_names = sorted(cons)
    ...
    def _validate_security_groups(self, mgr, data):
