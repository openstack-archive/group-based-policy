...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def validate_aim_mapping(self, mgr):
        ...
        self._ensure_common_tenant(mgr.aim_ctx)
        self._ensure_unrouted_vrf(mgr.aim_ctx)
        self._ensure_any_filter(mgr.aim_ctx)
        # self._setup_default_arp_dhcp_security_group_rules(mgr.aim_ctx)
        ...
        for net_db in mgr.session.query(models_v2.Network).all():
            if net_db.external:
                # REVISIT: Implement.
                print("Validating external network")
                # REVISIT: Get vmm_domains.
                l3out, ext_net, ns = self._get_aim_nat_strategy_db(
                    mgr.session, net_db)
                print(" l3out: %s" % l3out)
                print(" ext_net: %s" % ext_net)
                print(" ns: %s" % ns)
                if ext_net:
                    ns.create_l3outside(mgr.aim_ctx, l3out)
                    ns.create_external_network(mgr.aim_ctx, ext_net)
