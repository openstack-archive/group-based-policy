...
class ApicMechanismDriver(api_plus.MechanismDriver,
    ...
    def _dependent_port_change(self, port):
        ...
        if port and (port['device_owner'].startswith(
                n_constants.DEVICE_OWNER_DHCP) or
    ...
    def _notify_if_dependent_port_change(self, context, port):
        ...
        if self._dependent_port_change(port):
            ...
            # have opflex agents
            if not net or net[provider.NETWORK_TYPE] != ofcst.TYPE_OPFLEX:
                return
            ...
            affected_port_ids = [p['id'] for p in ports_to_update
                                 if not self._dependent_port_change(p)]
