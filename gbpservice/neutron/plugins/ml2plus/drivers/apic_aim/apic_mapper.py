...
class APICNameMapper(object):
    ...
    def mapper(name_type):
        ...
        def wrap(func):
            def inner(inst, context, resource_id, resource_name=None,
                      remap=False, prefix='', existing=False):
                if existing:
                    return ApicName(resource_id, resource_id, context, inst,
                                    func.__name__, existing=True)
                if remap:
                    inst.db.delete_apic_name(context.session, resource_id)
                else:
                    saved_name = inst.db.get_apic_name(context.session,
                                                       resource_id,
                                                       name_type)
                    if saved_name:
                        result = saved_name[0]
                ...
                name = ''
                try:
                    name = func(inst, context, resource_id, resource_name)
                ...
                if name:
                    ...
                    if inst.strategy == NAMING_STRATEGY_UUID:
                        result = inst._grow_id_if_needed(
                            context.session, purged_id, name_type, result,
                            start=inst.min_suffix)
                else:
                    result = purged_id

                inst.db.update_apic_name(context.session, resource_id,
                                         name_type, result)
                if prefix:
                    result = prefix + result
                    result = truncate(result, MAX_APIC_NAME_LENGTH)
    ...
    def tenant(self, context, tenant_id, tenant_name=None):
    ...
    def network(self, context, network_id, network_name=None):
        if not network_name:
    ...
    def delete_apic_name(self, context, object_id):
        self.db.delete_apic_name(context.session, object_id)
