...
gbp_opts = [
    cfg.BoolOpt('hybrid_mode',
                ...
                help=_("Whether Neutron's ports can coexist with GBP owned"
                       "ports.")),
    cfg.IntOpt('gbp_reserved_vlan',
               default=4094,
               help=_("Local vlan reserved for GBP owned ports.")),
...
class GBPOvsAgent(ovs.OVSNeutronAgent):
    ...
    def __init__(self, **kwargs):
        ...
        self.gbp_reserved_vlan = kwargs['gbp_reserved_vlan']
        ...
        del kwargs['gbp_reserved_vlan']
        del kwargs['epg_mapping_dir']
        super(GBPOvsAgent, self).__init__(**kwargs)
        # Remove GBP vlan from local vlan map
        self.available_local_vlans.discard(self.gbp_reserved_vlan)
    ...
    def port_bound(self, port, net_uuid,
        ...
        mapping = self.gbp_rpc.get_gbp_details(self.context,
                                               ...
                                               device=port.vif_id,
                                               host=cfg.CONF.host)
        if self.hybrid_mode and not mapping:
            ...
            if mapping:
                ...
                mapping = {'ptg_id': None}
            if mapping['ptg_id'] not in self.local_vlan_map:
                self.local_vlan_map[mapping['ptg_id']] = (
                    self.get_vlan_map_for_ptg(
                        mapping['ptg_id'], network_type, physical_network,
                        segmentation_id))
            lvm = self.local_vlan_map[mapping['ptg_id']]
            lvm.vif_ports[port.vif_id] = port
    ...
    def get_vlan_map_for_ptg(self, ptg_id, network_type, physical_network,
                             segmentation_id):
        # Same gbp vlan for each ptg_id.
        return ovs.LocalVLANMapping(
            self.gbp_reserved_vlan, network_type, physical_network,
            segmentation_id)
...
def create_agent_config_map(conf):
    ...
    agent_config['gbp_reserved_vlan'] = conf.OPFLEX.gbp_reserved_vlan
