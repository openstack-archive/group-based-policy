...
try:
    ...
    assert(getattr(sql_session.Session, 'GBP_PATCHED'))
except:
...
def new_writer(self):
    cwriter = self._clone(mode=enginefacade._WRITER)
    cwriter.orig_using = cwriter.using

    def new_using(context):
        orig_session = context.session
        pre_session()
        tscope = cwriter.orig_using(context)
        if not orig_session or orig_session != context.session or not(
                hasattr(context.session, 'notification_queue')):
            # Do not initialize the notification_queue if the
            # same session is being used (in the case of
            # nested writer calls)
            post_session(context.session)
        else:
            if sql_session.START_DEBUG:
                import pdb; pdb.set_trace()
            pass
        return tscope

    cwriter.using = new_using
    return cwriter


from oslo_db.sqlalchemy.enginefacade import _TransactionContextManager
setattr(_TransactionContextManager, 'writer', property(new_writer))
...
def get_reader_session():
    new_session = lib_db_api.get_context_manager().reader.get_sessionmaker()()
    return new_session
...
lib_db_api.get_reader_session = get_reader_session
"""
