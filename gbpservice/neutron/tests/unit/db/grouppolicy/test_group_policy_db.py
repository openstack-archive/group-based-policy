...
class GroupPolicyDBTestBase(object):
    ...
    def __getattr__(self, item):
        ...
        if item.startswith('update_'):
            ...
            if _is_gbpservice_resource(plural):
                def update_wrapper(id, **kwargs):
                    return self._update_gbpservice_resource(id, resource, **kwargs)
        ...
        if item.startswith('show_'):
            ...
            if _is_gbpservice_resource(plural):
                def show_wrapper(id, **kwargs):
                    return self._show_gbpservice_resource(id, plural, **kwargs)
                return show_wrapper
        # Create Method
        if item.startswith('create_'):
            ...
            if _is_gbpservice_resource(plural):
                def create_wrapper(**kwargs):
                    return self._create_gbpservice_resource(resource, **kwargs)
        ...
        if item.startswith('delete_'):
            ...
            if _is_gbpservice_resource(plural):
                def delete_wrapper(id, **kwargs):
                    return self._delete_gbpservice_resource(id, plural, **kwargs)
    ...
    def _create_gbpservice_resource(self, type, expected_res_status=None,
    ...
    def _update_gbpservice_resource(
    ...
    def _show_gbpservice_resource(self, id, plural, is_admin_context=False,
                           tenant_id=None):
    ...
    def _delete_gbpservice_resource(self, id, plural, is_admin_context=False,
...
class TestGroupResources(GroupPolicyDbTestCase):
    ...
    def _test_create_and_show(self, type, attrs, expected=None):
        plural = cm.get_resource_plural(type)
        res = self._create_gbpservice_resource(type, None, False, **attrs)
