...
class TestAIMServiceFunctionChainingBase(test_aim_base.AIMBaseTestCase):
    ...
    def tearDown(self):
        sfc_drivers = config.cfg.CONF.sfc.drivers
        LOG.warning(_LW("SFCDs used in this test: %s"),
                    sfc_drivers)
        flc_drivers = config.cfg.CONF.flowclassifier.drivers
        LOG.warning(_LW("FLCDs used in this test: %s"),
                    flc_drivers)
    ...
    def _create_simple_flowc(self, src_svi=False, dst_svi=False):
        ...
        def get_svi_kwargs(direction):
            l3o_name = uuidutils.generate_uuid()
            self.aim_mgr.create(self._aim_context, aim_res.L3Outside(
                tenant_name='common', name=l3o_name,
                vrf_name='openstack_UnroutedVRF'))
            extn = self.aim_mgr.create(
                self._aim_context, aim_res.ExternalNetwork(
                    tenant_name='common', l3out_name=l3o_name, name=direction))
            return {
                'apic:svi': True,
                cisco_apic.DIST_NAMES: {cisco_apic.EXTERNAL_NETWORK: extn.dn}
            }

        if src_svi:
            # We need to create the L3Out and the External network
            kwargs = get_svi_kwargs('src')
    ...
    def _verify_pc_mapping(self, pc):
        ...
        for flowc in flowcs:
            ...
            apic_tn = 'prj_' + dst_net['tenant_id'] if not dst_net[
                'apic:svi'] else 'common'
            device_clusters = []
