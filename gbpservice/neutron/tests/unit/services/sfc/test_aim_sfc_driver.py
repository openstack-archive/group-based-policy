...
class TestPortChain(TestAIMServiceFunctionChainingBase):
    ...
    def _verify_pc_mapping(self, pc):
        ...
        self.assertEqual(
            1, len(self.aim_mgr.find(ctx, aim_sg.ServiceGraph)))
        sg = self.aim_mgr.get(ctx, aim_sg.ServiceGraph(tenant_name=apic_tn,
                                                       name='ptc_' + pc['id']))
        ...
        for flowc in flowcs:
            ...
            for ppg in ppgs:
                ...
                dcic = aim_sg.DeviceClusterInterfaceContext(
                    tenant_name='common', contract_name=contract.name,
                ...
                self.assertEqual(dci.dn, dcic.device_cluster_interface_dn)
                self.assertNotEqual('', dcic.service_redirect_policy_dn)
        self.assertEqual(sorted({'name': x.name, 'device_cluster_name': x.name,
