...
class TemplateNodeDriverTestCase(
    ...
    def _create_network(self, fmt, name, admin_state_up, **kwargs):
        ...
        return super(TemplateNodeDriverTestCase, self)._create_network(
...
class TestServiceChainInstance(TemplateNodeDriverTestCase):
    ...
    def test_lb_external_redirect(self):
        expected_stack_template, provider = self._test_lb_node_create(
                                                    consumer_external=True)
        pt, pool_member = self._test_lb_dynamic_pool_member_add(
                                        expected_stack_template, provider)
        self._test_dynamic_lb_pool_member_delete(pt, pool_member,
                                                 expected_stack_template)
    ...
    def test_fw_node_east_west(self):
        ...
        with mock.patch.object(heatClient.HeatClient,
            ...
            stack_create.return_value = {'stack': {
                                        'id': uuidutils.generate_uuid()}}
            prs = self._create_fwredirect_ruleset(
    ...
    def test_fw_node_north_south(self):
        classifier_port = '66'
        classifier_protocol = 'udp'
        with mock.patch.object(heatClient.HeatClient,
                               'create') as stack_create:
            ...
            consumer_cidr = '172.0.0.0/22'
            routes = [{'destination': consumer_cidr, 'nexthop': None}]
            external_policy = self._create_external_policy(
            ...
            with mock.patch.object(heatClient.HeatClient,
                ...
                self.update_external_policy(
                                        ...
                                        consumed_policy_rule_sets={},
                                        expected_res_status=200)
                self.delete_external_policy(external_policy['id'],
                                            expected_res_status=204)
                stack_delete.assert_called_once_with(mock.ANY)
