...
from gbpservice.network.neutronv2 import api as g_api
from gbpservice.network.neutronv2 import client as g_client
...
class NeutronV2ApiTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.neutron_api = g_api.API()
        self.mock_client = mock.Mock(clientv20.Client)


    def _test_create_resource(self, resource):
        with mock.patch.object(g_client, 'get_client') as mock_get_client:
    ...
    def _test_router_interface(self, action):
        with mock.patch.object(g_client, 'get_client') as mock_get_client:
            mock_get_client.return_value = self.mock_client
            mock_client_action = getattr(self.mock_client, action+'_interface_router')
            mock_client_action.return_value = VALUE

            method_to_test = getattr(self.neutron_api, action+'_router_interface')
    ...
    # update security group rule is not in neutron yet
