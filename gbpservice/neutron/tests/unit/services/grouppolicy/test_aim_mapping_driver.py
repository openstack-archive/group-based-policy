...
class TestPolicyTargetGroup(AIMBaseTestCase):
    ...
    def _test_policy_target_group_lifecycle(self):
        ptg = self.create_policy_target_group(name="ptg1",
                                              tenant_id=self._tenant_id)
        ptg_id = ptg['id']
        self.show_policy_target_group(ptg_id, tenant_id=self._tenant_id,
                                      expected_res_status=200)

        ptg_id = ptg['id']
        ptg_name = ptg['name']
        aim_epg_name = self.name_mapper.policy_target_group(
        ...
        self.assertEqual(self._tenant_id, aim_epgs[0].tenant_name)
