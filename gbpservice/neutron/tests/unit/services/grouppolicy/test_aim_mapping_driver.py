            ...
            with self.subnet(network=network, cidr='1.1.2.0/24',
                ...
                with self.port(subnet=subnet) as port:
                    ...
                    self.assertEqual(address_scope['id'],
                                     mapping['l3_policy_id'])
                    vrf_name = self.name_mapper.address_scope(
                        self._neutron_context.session, address_scope['id'],
                        address_scope['name'])
                    self.assertEqual(vrf_name, mapping['vrf_name'])
                    self.assertEqual(epg_tenant, mapping['vrf_tenant'])
                    self.assertEqual(
                        set(['10.10.0.0/26', '1.1.0.0/16', '2.1.0.0/16']),
                        set(mapping['vrf_subnets']))
