...
class TestL2Policy(TestL2PolicyBase):
    ...
    def test_unshared_l2_policy_shared_l3p_cross_tenant(self):
        ...
        self.assertEqual(l2p['tenant_id'], bd.tenant_name)
        ...
        self.assertEqual(l3p['tenant_id'], bd.tenant_name)
        self.assertNotEqual(l2p['tenant_id'], bd.tenant_name)
        epg = self.driver._get_epg_by_dn(
            self._context, ptg['apic:distinguished_names']['EndpointGroup'])
        # EPG is now in L3P's tenant
        self.assertEqual(l3p['tenant_id'], epg.tenant_name)
        self.assertNotEqual(ptg['tenant_id'], epg.tenant_name)
