...
class TestPolicyTargetGroup(ApicMappingTestCase):
    ...
    def _test_policy_target_group_created_on_apic(self, shared=False):
        ...
        if self.single_tenant_mode:
            tenant = APIC_SINGLE_TENANT_NAME
        elif shared:
            tenant = self.common_tenant
        else:
            tenant = ptg['tenant_id']
        mgr = self.driver.apic_manager
        expected_calls = [
    ...
    def _test_ptg_policy_rule_set_created(self, provider=True, shared=False):
        ...
        ct_owner = cntr['tenant_id']
        if self.single_tenant_mode:
            ct_owner = APIC_SINGLE_TENANT_NAME
        elif shared:
            ct_owner = self.common_tenant
        tenant_id = ptg['tenant_id']
        if self.single_tenant_mode:
...
class TestL3Policy(ApicMappingTestCase):
    ...
    def _test_l3p_plugged_to_es_at_creation(self, shared_es,
        ...
        l3p_owner = l3p['tenant_id']
        if self.single_tenant_mode:
            l3p_owner = APIC_SINGLE_TENANT_NAME
        elif shared_l3p:
            l3p_owner = self.common_tenant
...
class TestPolicyRuleSet(ApicMappingTestCase):
    ...
    def _test_policy_rule_set_created_with_rules(self, shared=False):
        ...
        rule_owner = rules[0]['tenant_id']
        if self.single_tenant_mode:
            rule_owner = APIC_SINGLE_TENANT_NAME
        elif shared:
            rule_owner = self.common_tenant
        ctr_tenant = ctr['tenant_id']
        if self.single_tenant_mode:
            ctr_tenant = APIC_SINGLE_TENANT_NAME

        # Verify that the in-out rules are correctly enforced on the APIC
        mgr = self.driver.apic_manager
        expected_calls = [
            mock.call(ctr['id'], ctr['id'], rules[in_d]['id'],
                      owner=ctr_tenant, ransaction=mock.ANY,
    ...
    def _test_policy_rule_set_updated_with_new_rules(self, shared=False):
        ...
        rule_owner = old_rules[in_d]['tenant_id']
        if self.single_tenant_mode:
            rule_owner = APIC_SINGLE_TENANT_NAME
        elif shared:
            rule_owner = self.common_tenant
...
class TestPolicyRule(ApicMappingTestCase):
    ...
    def test_policy_classifier_updated(self):
        ...
        tenant_id1 = 'common'
        if self.single_tenant_mode:
            tenant_id1 = APIC_SINGLE_TENANT_NAME
        tenant_id2 = self._tenant_id
        if self.single_tenant_mode:
            tenant_id2 = APIC_SINGLE_TENANT_NAME
            ...
            for es in es_list:
                ...
                expected_assoc_calls.append(
                    mock.call(owner,
                              ...
                              transaction=mock.ANY))
                l3out = es['name' if self.pre_l3out else 'id']
                l3out_owner = self.common_tenant
                if self.single_tenant_mode:
                    l3out_owner = APIC_SINGLE_TENANT_NAME
                if self.pre_l3out:
