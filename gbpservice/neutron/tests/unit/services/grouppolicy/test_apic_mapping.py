...
class TestPolicyTarget(ApicMappingTestCase):
    ...
    def test_explicit_eoc_raises(self):
        ptg = self.create_policy_target_group(
            name="ptg1")['policy_target_group']
        self.assertRaises(
            amap.AdminOnlyOperation, self.create_policy_target_group,
            name="ptg2", description='opflex_eoc:')
        self.assertRaises(
            amap.AdminOnlyOperation, self.update_policy_target_group,
            ptg['id'], description='opflex_eoc:', is_admin_context=False)
