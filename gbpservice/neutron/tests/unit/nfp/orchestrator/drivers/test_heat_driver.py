...
from gbpservice.neutron.tests.unit.nfp.orchestrator.drivers import mock_dicts
...
HEAT_DRIVER_CLASS_PATH = ('gbpservice.nfp.orchestrator'
                          '.config_drivers.heat_driver')
...
@patch(OPENSTACK_DRIVER_CLASS_PATH + '.KeystoneClient.__init__',
       mock.MagicMock(return_value=Mock(roles=Mock(
           return_value=Mock(list=None)))))
@patch(HEAT_DRIVER_CLASS_PATH + '.HeatDriver._get_role_by_name',
       mock.MagicMock(return_value='admin-role'))
class TestHeatDriver(unittest.TestCase):
    ...
    def test_keystone(self, mock_obj):
        ...
        keystone_client._get_v2_keystone_admin_client = 'abcd123'
    ...
    @mock.patch.object(KeystoneClient, '_get_v2_keystone_admin_client')
    @mock.patch.object(heat_driver.HeatDriver, '_get_role_by_name')
    def _initialize_heat_object(self, mock_get_role, v2_client):
        mock_get_role.return_value = 'admin-role'
        v2_client_mock = Mock(users=Mock(find=Mock(id=100)))
        v2_client.return_value = v2_client_mock
