...
class NfpController(nfp_launcher.NfpLauncher, NfpService):
    ...
    def reload_config_files(self):
        """To reload config files.
            ...
            It will reload config files of its own conf object and
            also create an event to reload config files and
            send the event to all child process.
        """
        LOG.debug("Reloading config files")
        self._conf.reload_config_files()
        event = self.new_event(id='RELOAD_CONFIG_FILES')
        event.desc.type = None
        childs = self.get_childrens()
        for pid, wrapper in childs.iteritems():
            pipe = wrapper.child_pipe_map[pid]
            self.pipe_send(pipe, event)
