...
class PollingTask(oslo_periodic_task.PeriodicTasks):
    ...
    def periodic_sync_task(self, context):
        LOG.debug("Periodic sync task invoked !")
...
class PollQueueHandler(object):
    ...
    def _cancelled(self, ev):
        ...
        """
        LOG.info("Poll event %s cancelled" % (ev.identify()))
    ...
    def _schedule(self, ev):
        ...
        """
        LOG.debug("Schedule event %s" % (ev.identify()))
        eh = self._ehs.get(ev)
    ...
    def _process_event(self, ev):
        ...
        LOG.debug("Processing poll event %s" % (ev.identify()))
    ...
    def _event_done(self, ev):
        ...
        """
        LOG.info("Poll event %s to be marked done !" % (ev.identify()))
        self.remove(ev)
    ...
    def add(self, event):
        ...
        """
        LOG.debug("Add event %s to the pollq" % (event.identify()))
    ...
    def remove(self, event):
        ...
        """
        LOG.info("Remove event %s from pollq" % (event.identify()))
        LOG.info(
    ...
    def fill(self):
        """Fill polling cache with events from poll queue.
            ...
            or cancelled.
        """
        LOG.debug("Fill events from multi processing Q to internal cache")
    ...
    def peek(self, idx, count):
        """Peek for events instead of popping.
            ...
            are not lost.
        """
        LOG.debug("Peek poll events from index:%d count:%d" % (idx, count))
        cache = self._cache.copy()
        qlen = len(cache)
        LOG.debug("Number of elements in poll q - %d" % (qlen))
