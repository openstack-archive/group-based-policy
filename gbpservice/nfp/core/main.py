...
class EventHandlers(object):
    ...
    def register(self, event_desc):
        LOG.debug("Registering handler %s" % (self.identify(event_desc)))
    ...
    def get(self, event):
        for id, eh in self._ehs.iteritems():
            if id == event.id:
                LOG.debug("Returning handler %s" % (self.identify(eh[0])))
                return eh[0].handler
        return None
...
class Controller(object):
            ...
            for val in seq.values():
                if val['in_use']:
                    ...
                    if val['queue'] == []:
                        continue
                    event = val['queue'][0]
                    LOG.debug("Returing serialized event %s"
                              % (event.identify()))
                    return event
        return None
    ...
    def modules_init(self, modules):
        ...
        for module in modules:
            LOG.info("Initializing module %s" % (identify(module)))
            try:
                module.module_init(self, self._conf)
            except AttributeError:
                LOG.error("Module %s does not implement"
    ...
    def start(self):
        ...
        for worker in self._workers:
            worker[0].start()
            LOG.debug("Started worker - %d" % (worker[0].pid))
    ...
    def post_event(self, event):
        ...
        LOG.info("Scheduling internal event %s"
                 "to worker %d"
    ...
    def event_done(self, event):
        ...
        """
        LOG.info("Event %s done" % (event.identify()))
        seq_map = self._sequencer.copy()
        ...
        LOG.debug("Checking if event %s in serialize Q"
                  % (event.identify()))
        seq_q = seq_map[event.binding_key]['queue']
        for seq_event in seq_q:
            if seq_event.key == event.key:
                LOG.debug("Removing event %s from serialize Q"
                          % (seq_event.identify()))
    ...
    def poll_event(self, event, max_times=sys.maxint):
        ...
        """
        LOG.info("Adding to pollq - event %s for maxtimes: %d"
    ...
    def poll_event_done(self, event):
        ...
        """
        LOG.info("Poll event %s done.. Adding to pollq"
                 % (event.identify()))
    ...
    def register_events(self, events):
        ...
        for event in events:
            LOG.info("Registering event %s & handler %s"
    ...
    def init_complete(self):
        ...
        for module in self._modules:
            LOG.info("Invoking init_complete() of module %s"
                     % (identify(module)))
            try:
                module.init_complete(self, self._conf)
            except AttributeError:
                LOG.info("Module %s does not implement"
