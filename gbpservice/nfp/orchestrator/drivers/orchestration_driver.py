...
class OrchestrationDriver(object):
    ...
    def _verify_meta_data(self, image_name, metadata):
        meta_data = {}
        try:
            for attr in metadata:
                if attr in nfp_constants.METADATA_SUPPORTED_ATTRIBUTES:
                    meta_data[attr] = ast.literal_eval(metadata[attr])
        except Exception as e:
    ...
    def _get_meta_data(self, device_data, image_name):
        token = self._get_token(device_data.get('token'))
        if not token:
            return None
        try:
            ...
            return None
        meta_data = self._verify_meta_data(image_name, metadata)
        if not meta_data:
            return {}
        return meta_data

    def _get_meta_data_fast(self, token,
    ...
    def _update_self_with_meta_data(self, meta_data, attr):
        attr_value = getattr(self, attr)
        if attr in meta_data:
            setattr(self, attr, meta_data[attr])
        else:
    ...
    def _update_meta_data(self, device_data, token=None):
        meta_data = {}
        try:
            image_name = self._get_image_name(device_data)
            meta_data = self._get_meta_data(device_data, image_name)
            LOG.info(_LI("Vendor data, specified in image: %(meta_data)s"),
                     {'meta_data': meta_data})
            if meta_data:
                self._update_self_with_meta_data(
                    meta_data,
    ...
    def _update_meta_data_fast(self, token, admin_tenant_id,
                               image_name, device_data):
        meta_data = None
        try:
            meta_data = self._get_meta_data_fast(
    ...
    def create_network_function_device(self, device_data,
        ...
        pre_launch_executor.add_job('UPDATE_VENDOR_DATA',
                         self._update_meta_data_fast,
                         token, admin_tenant_id, image_name, device_data,
                         result_store=meta_data_result)
        ...
        try:
            ...
            if meta_data.get('supports_hotplug') is False:
    ...
    def _validate_pre_launch_executor_results(self, network_handler,
        ...
        meta_data = meta_data_result.get('result', None)
        if not meta_data:
            LOG.warning(_LW('Failed to get vendor data for device creation.'))
            meta_data = {}

        return interfaces, image_id, meta_data
    ...
    def unplug_network_function_device_interfaces(self, device_data,
        ...
        if image_name:
            meta_data = (
                self._update_meta_data_fast(token,
                                            device_data['tenant_id'],
                                            image_name,
                                            device_data))
