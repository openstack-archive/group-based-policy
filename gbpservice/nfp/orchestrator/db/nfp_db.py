...
class NFPDbBase(common_db_mixin.CommonDbMixin):
    ...
    def _set_meta_data_for_nfd(self, session,
                               network_function_device_db,
                               network_function_device,
                               is_update=False):
        nfd_db = network_function_device_db
        meta_data = nfd_db['meta_data']

        if is_update:
            if meta_data:
                meta_data = jsonutils.loads(meta_data)
            updated_meta_data_str = network_function_device.pop(
                    'meta_data', {})
            if not updated_meta_data_str:
                return
            if updated_meta_data_str:
                updated_meta_data = jsonutils.loads(updated_meta_data_str)
            if (type(updated_meta_data) is dict
                    and updated_meta_data and meta_data):
                updated_meta_data.update(meta_data)
            meta_data_str = jsonutils.dumps(updated_meta_data)
        else:
            if not meta_data:
                meta_data_str = ''
                return
            meta_data_str = jsonutils.dumps(meta_data)
        nfd_db.meta_data = meta_data_str
    ...
    def update_network_function_device(self, session,
        ...
        with session.begin(subtransactions=True):
            ...
            if updated_network_function_device.get('meta_data'):
                updated_network_function_device[
                        'meta_data'] = jsonutils.dumps(
                                updated_network_function_device['meta_data'])
    ...
    def _make_network_function_device_dict(self, nfd, fields=None):
        ...
        if nfd.get('meta_data'):
            res.update({'meta_data': nfd['meta_data']})
        return res
