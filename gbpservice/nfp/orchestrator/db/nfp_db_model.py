...
class NetworkFunctionDevice(BASE, model_base.HasId, model_base.HasTenant,
    ...
    vendor_data = sa.Column(sa.String(1024), nullable=True)
...
class HAInfo(BASE, model_base.HasId, model_base.HasTenant):
    """
    This table contains info about the ports participating in HA and optional.
    """
    __tablename__ = 'nfp_ha_mapping_info'
    network_function_device_id = sa.Column(sa.String(36), nullable=False)
    vrrp_group = sa.Column(sa.Integer(), nullable=True)
    vip_ip = sa.Column(sa.String(36), nullable=True)
