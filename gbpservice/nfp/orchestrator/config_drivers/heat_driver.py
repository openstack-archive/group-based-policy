...
class HeatDriver(object):
    ...
    def _resource_owner_tenant_id(self):
        user, pwd, tenant_name, auth_url = utils.get_keystone_creds()
    ...
    def _get_heat_client_fast(self, tenant_id, assign_admin=False):
        if assign_admin:
            try:
    ...
    def check_config_complete(self, nfp_context):
        ...
        heatclient = self._get_heat_client_fast(provider_tenant_id)
        if not heatclient:
            return failure_status
        try:
            stack = heatclient.get(stack_id)
